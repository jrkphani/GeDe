...
## 5. Enable SES sending — and, in the same merge, the localised code mail

Cognito sends one-time codes with its built-in sender (`EmailSendingAccount: COGNITO_DEFAULT`, `no-reply@verificationemail.com`, 50 per day) until SES has production access. What it sends is already GeDe's: the pool's own templates carry the branded en-US layout from `@gede/mail` (ADR-044). The **localised** version — the custom-message trigger that renders the same layout in the user's language — exists in the Auth stack but is **not attached** to the pool: the developer guide states that under `COGNITO_DEFAULT` a trigger response carrying `emailMessage`/`emailSubject` makes Cognito answer the caller with `InvalidLambdaResponseException` — no code delivered, every email sign-up and sign-in refused, and the handler cannot see it (#158 review). **Never attach the trigger while the sender is Cognito's.** Both flips happen in one merge:

1. Request production access in `ap-southeast-1`: `aws sesv2 put-account-details --production-access-enabled --mail-type TRANSACTIONAL --website-url https://gede.work --use-case-description "..."`. Wait for the approval email (§16 records the first request was denied; re-request with the fuller description there).
...
3. In one branch: in `infra/lib/stacks/auth-stack.ts` replace `email: cognito.UserPoolEmail.withCognito()` with the `withSES({ fromEmail, fromName, sesVerifiedDomain, sesRegion })` block that sits commented directly beneath it, **and** set `"customMessageTrigger": true` in `infra/cdk.json`. `npm test -w infra` (the flag-on describe in `stage.test.ts` asserts `LambdaConfig.CustomMessage` and the invoke permission). Merge. The templates are sender-independent: the From address moves to `no-reply@gede.work` and the codes start arriving in the user's locale; nothing else changes.
4. After the deploy: `aws cognito-idp describe-user-pool` shows `EmailSendingAccount: DEVELOPER` and `LambdaConfig.CustomMessage`; sign up with a test address and confirm the branded "Your GeDe sign-up code" arrives from `no-reply@gede.work`, sign in with a code and confirm "Your GeDe sign-in code", then set the account's language to தமிழ் and repeat the sign-in for the Tamil mail. A stock-looking English mail after that means the trigger returned the event untouched — its log group `GeDe-Prod-Auth-CustomMessageLogs` says why; the `gede-prod-custom-message-errors` alarm fires only when the handler itself failed. An `InvalidLambdaResponseException` on sign-up or sign-in means the sender is not `DEVELOPER` after all: set `customMessageTrigger` back to `false` and merge at once.
...
### SES events: bounces and complaints (ADR-046)

What the production-access case was told — "bounces and complaints go to an SNS topic; a hard bounce or complaint withdraws the invitation and blocks further mail to that address" — is wired as follows, all in `GeDe-Prod-Auth` except the consumer:

| Piece                                           | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Configuration set `gede-prod`                   | The identity's default set (`ConfigurationSetAttributes` on `gede.work`), so every `SendEmail` from the domain carries it. Reputation metrics on; bounces and complaints added to the suppression list. `withSES` names it too (`configurationSetName`), so Cognito's codes report through it once the pool sends through SES.                                                                                                                                                                      |
| Event destination `gede-prod-ses-events`        | Publishes `bounce`, `complaint` and `reject` to SNS topic `gede-prod-ses-events`. Not `send`/`delivery` (a message per mail for nothing) and not `deliveryDelay` (SES retries; a delay that ends badly arrives as a bounce).                                                                                                                                                                                                                                                                        |
| SQS `gede-prod-ses-events` (+ `-dlq`)           | Raw subscription to the topic (the body is the SES event). SSE-SQS, TLS-only, 14-day retention, 60 s visibility, dead-letter after 5 receives. The service task role may `ReceiveMessage`/`DeleteMessage`/`GetQueueAttributes` on it and nothing else; the jobs task has no grant.                                                                                                                                                                                                                  |
| The poller (`services/sync/src/mail/events.ts`) | Runs on the service task whenever `SES_EVENTS_QUEUE_URL` is set (the task definition sets it; locally it is unset and nothing runs). Long-polls 20 s. Per event and recipient: a row in `mail_events` (primary key = SES message id + address, so a redelivery is a no-op), then the verdict — a `Permanent` bounce or a complaint suppresses at once; a `Transient`/`Undetermined` bounce on the third within 30 days; a reject is recorded only (SES refused our content, not their address).     |
| `mail_suppressions`                             | The addresses GeDe will not mail again (`reason` = `bounce` \| `complaint`, first and last sighting, the SES event as `source`). Writing the row withdraws every pending invitation to the address with a `share.invite_withdraw` audit row by the system (target `<address>:<reason>`). `POST …/invites` (no account) and `…/resend` answer 409 `address_suppressed` — the sheet shows "This address cannot receive email from GeDe"; a member with an account is still shared with, mail skipped. |
| Account-level suppression list                  | `BOUNCE` + `COMPLAINT`, pinned by the `Custom::GedeSesAccountSuppression` resource (`PutAccountSuppressionAttributes` on create and update; nothing on delete). SES's own list is the backstop under ours: a suppressed address is refused by SES itself even if a row here were removed.                                                                                                                                                                                                           |
| Metric `GeDe/Sync MailEvents`                   | One datapoint per event, `Reason` = `bounce_permanent` \| `bounce_transient` \| `complaint` \| `reject` \| `unrecognised`. Log lines carry the SES message id and the verdict, never the address.                                                                                                                                                                                                                                                                                                   |
| Alarms                                          | `gede-prod-ses-bounce-rate` (> 5 %), `gede-prod-ses-complaint-rate` (> 0.1 %) on SES's account `Reputation.*` metrics — AWS's review thresholds; `gede-prod-ses-events-dlq` on any message in the dead-letter queue (§8).                                                                                                                                                                                                                                                                           |

```bash
# What is suppressed, and why (as the master user or the app role; the addresses are personal data — do not paste them anywhere)
psql "$DATABASE_URL" -c "select email, reason, first_seen_at, last_event_at from mail_suppressions order by last_event_at desc"
# The address's history
psql "$DATABASE_URL" -c "select message_id, kind, at from mail_events where email = 'person@example.com' order by at"
# SES's own account-level list
aws sesv2 list-suppressed-destinations --query 'SuppressedDestinationSummaries[].{email:EmailAddress,reason:Reason,at:LastUpdateTime}'
# The queue and its dead letters
Q=$(aws sqs get-queue-url --queue-name gede-prod-ses-events --query QueueUrl --output text)
D=$(aws sqs get-queue-url --queue-name gede-prod-ses-events-dlq --query QueueUrl --output text)
aws sqs get-queue-attributes --queue-url "$D" --attribute-names ApproximateNumberOfMessages
aws sqs receive-message --queue-url "$D" --max-number-of-messages 10 --visibility-timeout 0 --query 'Messages[].Body'   # peek; nothing is deleted
```

**Un-suppress an address** (the person fixed their mailbox, or the complaint was a mistake) — both halves, or SES keeps refusing:

```bash
psql "$DATABASE_URL" -c "delete from mail_suppressions where email = 'person@example.com'"
aws sesv2 delete-suppressed-destination --email-address person@example.com
```

The invitation that was withdrawn is gone; the sharer sends a new one (the sheet lists it under Add people). `mail_events` keeps the history: three transient bounces within 30 days would suppress again.

**A message on the dead-letter queue** means the service failed to process it five times — read the service log for `ses event not processed; left for redelivery` with the `messageId`, fix the cause (usually the database), then redrive: `aws sqs start-message-move-task --source-arn <dlq arn> --destination-arn <queue arn>`. The poller is idempotent: whatever it had recorded before the failure is recognised as a duplicate, the suppression is an upsert, and the withdrawal finds nothing left to withdraw. A body the poller cannot read is never dead-lettered: it is counted `unrecognised`, logged with the reason, and deleted.
...
| `gede-prod-ses-bounce-rate`          | SES account `Reputation.BounceRate` > 5 % (hourly, AWS's review line) | Addresses are being mailed that bounce. §5 "SES events": is the poller running (`ses events poller` line at boot, `GeDe/Sync MailEvents` datapoints), is the dead-letter queue empty, does `mail_suppressions` hold the bouncing addresses? At 10 % AWS pauses sending.                              |
| `gede-prod-ses-complaint-rate`       | SES account `Reputation.ComplaintRate` > 0.1 % (hourly)               | Someone marked GeDe mail as spam. Same checks; expect the address in `mail_suppressions` with `reason = complaint` and its invitations withdrawn. At 0.5 % AWS pauses sending.                                                                                                                       |
| `gede-prod-ses-events-dlq`           | ≥ 1 message on `gede-prod-ses-events-dlq`                             | An SES event the service could not process five times: that address may still be mailed. Peek at the message, read the service log for `ses event not processed`, fix, redrive (§5).                                                                                                                 |
...
| CodeConnections connection     | `arn:aws:codeconnections:ap-southeast-1:975049998516:connection/7e4906e9-e0c6-4c1a-bcf7-ebc69f842097` (`gede-github-2`), in `infra/cdk.json`. The first connection went to ERROR on 2026-09-13 ("authorization revoked or expired"); an ERROR connection cannot be repaired — create a new one, authorise it in the console, update `cdk.json`, and deploy `GeDe-Pipeline` once from a laptop (Source cannot run on the dead connection). | 2026-09-13 |
