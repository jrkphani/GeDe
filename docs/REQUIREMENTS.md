...
- **MENU-04** — Cell menu: Freeze; add row above/below; add column before/after; delete row/column; sort, filter, category options; merge controls; cut, copy, copy snapshot, paste, paste and match style, clear all; wrap text. _Amended by ADR-051: Fit column width to content beside Fit row height to content, so a column fits with the header row hidden; the table's context menu (on the title) carries Rename table… (F2)._
