...
| Identity      | Cognito user pool, `featurePlan: ESSENTIALS`, `AllowedFirstAuthFactors: [EMAIL_OTP, WEB_AUTHN]`, WebAuthn relying party id `gede.work`, no password factor. SPA client with `USER_AUTH` flow only. Email via Cognito's built-in sender until SES leaves sandbox. Apple IdP wired behind CDK context `appleSignIn`.                                                       |
