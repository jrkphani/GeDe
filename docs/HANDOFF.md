...
- **P6 — deploy-gate transition (the careful one — do NOT freeze deploys). ← START HERE.** FIRST de-flake the 096 `test.fixme` quarantined specs (`e2e/d3-canvas.spec.ts`: popover-anchor + focus-pan) with deterministic pan-settle/zoom-invariant asserts, and prove green across N consecutive runs while STILL `@dev-flag`. THEN drop the `@dev-flag` tags + `--grep-invert @dev-flag` (`package.json:14`) + retire `.github/workflows/dev-canvas-e2e.yml`; invert the RF zero-size bundle guard into a budget CEILING (RF ships ~88 KB now). The `?d3rf` spec count is now **22 `@dev-flag` tests (+1 fixme)** across P0–P5 — all must be green in the gating `verify` before un-tagging. Watch `gh run list --json` so a flake never freezes prod deploy (the 096 failure mode). Do P6 immediately before P7.
