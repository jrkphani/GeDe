...
- **② 089 — unified canvas workspace** (`docs/issues/089-unified-canvas-workspace.md`): **now unblocked — 090 shipped its prerequisite** (canvas is a real entity; 089 renders N canvases as N React-Flow clusters). Merge the 3 tier routes into one `@xyflow/react` zoomable canvas, positions derived via dagre/elk auto-layout from ordinal `sort` (no x/y persisted). **089-D1 (global rich-text toolbar) is independent, high-value, low-risk** — a good standalone next build.
