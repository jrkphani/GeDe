...
- **e2e canvas/fallback SPLIT**: the ~57 non-`@dev-flag` specs re-pin to the shipping `WorkspaceSurface` fallback via `e2e/workspaceSurface.ts::forceWorkspaceSurface` (a `prefers-reduced-data` matchMedia shim); `d3-canvas.spec.ts` (now **25 `@dev-flag`** tests, incl. 2 canvas axe smokes + the 101 click/touch-no-pan pair) is the canvas suite. `playwright.config.ts` has `retries: CI ? 2 : 0`.
...
- **`099` — canvas-default coverage + a11y follow-ups.** DONE so far (`76ebf08`): cross-node-Tab flake hardened (099a); canvas axe smoke extended to a populated register (099b); label-tier-vs-zoom → **NON-ISSUE, closed**. **Two real bugs FOUND, still open — the concrete next items:** (2b) **CRITICAL** — `CoverageMatrix.tsx:388/441` has absolutely-positioned `role="gridcell"` with no `role="row"` parent → invalid ARIA grid on BOTH surfaces; fix with `display:contents` `role="row"` wrappers, then re-enable the deferred coverage-twin axe scan. (2c) `Canvas.tsx:130` hit-radius is transform-blind → the ~44px dot target shrinks under zoom (WCAG 2.5.5); feed screen-space width. Still-open coverage: canvas-side e2e for hover-mute + empty-state suppression (currently fallback-only); touch/tablet verification. All small/independent — see issue 099.
...
*History (shipped + archived to `docs/issues/done/`): 084 grid unification; 087/088/090/091/092/094/095/096/097/098; 089 (D1/D2/D3-graduation) + 093. SHIPPED but not yet archived: 101 (click-no-pan). This HANDOFF was updated 2026-07-20 after the 101 fix.*
