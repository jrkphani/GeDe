...
**v2 (collaboration) — PR #7, merged + deployed this session.** The whole v2 wave (issues **032, 033, 034, 035, 036, 043**) plus **037** (local→cloud on-ramp) and **042** (semantic palette) is integrated on branch `feat/v2-collaboration-wave` → **PR #7**, all CI green (`verify` 771 unit + 49 e2e, `cdk-validate` 65 CDK, `migration-parity`; migrations `0000`–`0011`). Merging auto-deploys the v2 stacks (NAT/RDS/Fargate from 030), RLS (0008), invitations (0009), write-path Lambda + ledger (0010), the adopted-project column (0011), and the new static build. The v2 cost gate (RDS/NAT ~$30–60/mo) began at that merge.
...
- **042** **Semantic ⌘K palette** — client-side embeddings ($0 AWS): `@huggingface/transformers` MiniLM in a separate 549 kB lazy chunk (main bundle +4 kB), blended with 017's lexical ranking. Model auto-load is gated by `VITE_SEMANTIC_SEARCH` (Playwright sets it `off` so e2e has no network dep); load failure degrades to lexical-only.
...
**Next up:** the entire backlog (000–043) is shipped. **038** (presence) shipped as a cross-**tab** slice only (BroadcastChannel) — real cross-network presence needs new server transport (WebSocket/Electric primitive) and demand-validation first. See `docs/issues/README.md` + ADR-0008/0009/0010. Two known follow-ups from this wave: wire the client optimistic-write queue (032) to actually flush to the write-path API (043) — no client→server HTTP write exists yet; and swap 043's inline-stub Lambda for the real bundled handler + wire 033's Cognito issuer.
