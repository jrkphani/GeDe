...
**v2 (collaboration) — merged to `main` and LIVE.** The whole v2 wave (issues **032, 033, 034, 035, 036, 037, 038, 042, 043**) is merged and **deployed to production** (via PR #7 then follow-on pushes). All **6 CloudFormation stacks** (`Network`/`Data`/`Api`/`Hosting`/`Auth`/`Dns`) are `*_COMPLETE` and the app serves **HTTP 200** at https://d1nzod71m3rz6x.cloudfront.net. Migrations `0000`–`0011`; final `npm run verify` **810 unit + 49 e2e**, CDK **65**. The v2 cost gate (RDS + NAT, ~$30–60/mo) is now **active**. `main` == `origin/main`.

**Deploy gotcha paid for:** the first deploy hung ~45 min then the CloudFront **`BucketDeployment` Lambda OOM'd** (default 128 MB) — 042 self-hosts the ~22 MB onnxruntime WASM, pushing `dist/` to ~38 MB. Fixed by `memoryLimit: 1024` in `deploy/cdk/lib/hosting-stack.ts`. `cdk deploy --all` runs stacks **sequentially**, so a stuck stack blocks the rest; watch CloudFormation directly (`describe-stacks`) when a deploy runs long.
...
- **037** **Local→cloud on-ramp** — on sign-in, adopt local PGlite projects into the cloud workspace (reuses 015's envelope + `importProject`, assigns `workspace_id`, enqueues onto 032's queue; migration `0011` `adopted_into_project_id`). Also repaired the stale `0010` snapshot chain 043 left behind.
- **042** **Semantic ⌘K palette** — client-side embeddings ($0 AWS): `@huggingface/transformers` MiniLM, blended with 017's lexical ranking. The ~22 MB onnxruntime WASM is **self-hosted in `dist/`** (lazy-fetched by the client — no external CDN dependency). Model auto-load is gated by `VITE_SEMANTIC_SEARCH` (Playwright sets it `off` so e2e has no network dep); load failure degrades to lexical-only.
- **038** **Presence** — ephemeral roster + focus/editing cues over a `BroadcastChannel` seam (DI-testable like 032's sync). No migration, no deps. **Cross-tab only**; cross-network needs new server transport (WebSocket/Electric primitive). Register cue only (canvas unwired). Speculative — "demand validated" gate left unchecked.
...
**Next up:** the **entire backlog (000–043) is shipped and live** — no OPEN issues. What remains are follow-ups (file as new issues when wanted), highest-leverage first:
1. **Close the write loop** — wire 032's client optimistic-write queue to actually flush to 043's `/write` API (no client→server HTTP write exists yet; both were built to a documented seam), and swap 043's inline-stub Lambda for the real bundled handler + wire 033's Cognito issuer into the Api stack (cross-stack ref).
2. **Google Workspace federation** (033 fast-follow — Cognito IdP config).
3. **Cross-network presence** (038) — real transport + demand validation.
4. **Extend `workspace_id` denormalization** to `tier2_entries`/`parameters`/`bindings` if the join-scoped RLS proves limiting (034 tradeoff).
See `docs/issues/README.md` + ADR-0008/0009/0010.
