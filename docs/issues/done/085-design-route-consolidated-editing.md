...
- **Framing partially superseded — issue 089-D3 P7 (2026-07-20).** The workspace-canvas graduation makes the pan/zoom canvas the **primary organizing surface**, which **reverses 085's *spirit*** ("the canvas is a visual you glance at, the tables are the instrument" — decision 3). BUT 085's **narrower, load-bearing rule still HOLDS**: there is **no on-ring authoring** — the ring stays a *derived* visual and the **register is the authoring surface** inside the Design core (089-P2). So this is a *framing* reversal only; it does **NOT** re-open **082 Phase 2** (on-ring editing — ghost "+" gap segments / on-ring rename), which stays formally shelved (see **Supersedes** below). All other 085 decisions (one editing zone, keyboard-continuous rail→register, no redundant Composer strip, proportional even-fill ring) stand unchanged.
