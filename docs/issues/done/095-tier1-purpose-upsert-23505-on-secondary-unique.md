...
- **Status**: ✅ SHIPPED + **DEPLOYED + LIVE-VERIFIED 2026-07-18** (`index-yIDo1fKM.js`). Archived to `done/`. The 23505 is gone.
- **Live verification** (server-side, authoritative): after deploy, CloudWatch (`…WriteApiFunction…`) shows **ZERO `tier1_purpose_project_idx` 23505 errors** despite active `tier1_purpose` write traffic (pre-deploy the exact same project 019f69a0 logged many 23505s at 09:35–09:41 UTC); write invocations complete cleanly. The purpose edit that used to 500 now persists. (Client-side `page.on` couldn't observe the raw 200 on the new build's SW — a smoke limitation — but the server log is decisive.)
- **⚠ Residual — the SAME root cause also causes 091 (`unknown_entity`), so 095 is only HALF the fix:** the client mints a fresh `tier1_purpose` id (cold mirror). This fix makes the **`upsert`/insert** reconcile onto the server's existing row via `ON CONFLICT (project_id) DO UPDATE` — so it no longer 23505s **and the edit persists** — but the server row keeps ITS OWN id, so the client's minted id still diverges. The **next** edit enqueues an `update` for the client's id, which the server has no record of → `checkTenancy` `unknown_entity` → the **091** note (reproduced live at 11:50 UTC by this session's own 095 smoke: `{reason:unknown_entity, table:tier1Purpose, op:update, entityId:019f750f…}`). **The complete fix is in 091: resolve `tier1_purpose` by its natural key (`project_id`) for `update` + `checkTenancy` too, not just the insert** (or have the client adopt the server row's id). 095 stands as a real net win — data now persists instead of being lost to a 500 — but the id-divergence must be closed in 091.
- ~~**Status**: ✅ FIX LANDED (red-first) — **pending deploy + live-verify**.~~ Discovered 2026-07-18 while live-verifying 087 (087's write-stall footer surfaced this genuine failure in the wild); owner chose to fix it next.
- **Fix**: `src/server/writeApi/store.ts` — a `NATURAL_KEY_CONFLICT` map routes `tier1_purpose` inserts to `ON CONFLICT (project_id) DO UPDATE SET …EXCLUDED…` (a real upsert that reconciles a cold-mirror client's fresh `id` onto the existing row and persists the edit), instead of the id-only `ON CONFLICT (id) DO NOTHING` that 23505'd on the secondary unique index. Red-first: `pgWriteStore.contract.test.ts` (SQL-shape, runs in `verify:fast`) + a scope guard that other tables keep `ON CONFLICT (id) DO NOTHING`, and an authoritative real-Postgres case in `pgWriteStore.live.test.ts` (guard-skips without a DB — docker was unavailable locally). `verify:fast` green (1504). **Deploy + live re-verify (purpose edit now 200s, not 23505) still owed.**
- **Scope note**: only `tier1_purpose` is fixed (the reproduced bug). `bindings` / `workspace_members` / `canvases` share the theoretical secondary-unique exposure but want *different* conflict semantics (a binding is an idempotent link → `DO NOTHING`, not `DO UPDATE`) and were not reproduced — left as an explicit audited follow-up in the map's doc comment.
