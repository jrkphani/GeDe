...
- **Status**: ✅ **FIX LANDED (2026-07-19)** — owner chose the **dedicated `revive` op** (of the 3 options below). Shipped in two phases, each adversarial-review-passed + real-Postgres-verified:
  - **Phase 1 (server, deployed):** a new `revive` mutation op un-tombstones a soft-deleted row (or inserts it live if absent), idempotent + cross-tenant-safe + natural-key-aware. `MutationOp` gains `'revive'`; `checkTenancy` resolves the target via `resolveWorkspaceForEntityIncludingDeleted` (does NOT filter `deleted_at`; range-checks a tombstoned target → `cross_tenant`; absent → allow-as-insert); `applyIfNew` is UPDATE-first + INSERT-if-absent (NOT `ON CONFLICT DO UPDATE` — a real-PG probe proved NOT NULL fires on the tentative INSERT tuple before the arbiter, breaking partial-payload revives) and natural-key-addressed for `tier1_purpose` (else a diverged-id revive 23505s — an adversarial-review catch). Skips the LWW gate (like insert).
  - **Phase 2 (client):** every command-log undo/redo closure that RESURRECTS a tombstoned row (undo-of-remove, redo-of-add, restore-from-archive incl. the forward **070** cloud-restore bug, + cascade-restored bindings) now enqueues `'revive'`; field-edit/reorder/rebind-of-live-row reversals stay `'update'` (revive would wrongly bypass LWW); undo-of-add stays `'delete'`. `QueuedMutation`/`writeTransport` carry the new op. Also folded in the same-class `revertStale` bug (its re-inserted tombstoned bindings → `'revive'`).
  - The original "no undo/redo closure enqueues" gap (below) is subsumed — every reversal now enqueues the correct op to the outbox.
