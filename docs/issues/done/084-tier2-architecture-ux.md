...
- **Status**: **✅ COMPLETE — Direction 3 grid unification SHIPPED (P0–P6, 2026-07-19).** All 8 design forks + 3 UX requirements were answered in the fork-interview, then built out red-first across P0–P6 (spec: **`## Direction 3 — APPROVED BUILD PLAN (2026-07-19)`** below). **P0** shared `gridBoundaryFocus` seam (`313551d`); **P1** outer `EditableChainProvider` + trailing add-table phantom + indentation/weight (`4936792`); **P2** cross-table Tab via the seam (`e78f4e3`); **P3** inline typed add-child (`203b527`) + per-depth child indentation (`bac729f`); **P4** listbox selection a11y + quiet Remove (`2d0f55e`); **P5** quiet aria-hidden keyboard-shortcut hints (`a77291c`, LIVE via `acece4c`/`index-CJUxFg34.js`); **P6** e2e + a11y sweep (`e6ba1eb`) — keyboard-only build/thread flow, listbox+`aria-selected` promote, hints reveal, and a **20×50 volume** test (bulk-seeded via the `.gede.json` import path; deep cell-open <3s; cross-table focus intact) — retiring the perf-at-volume risk. **P6's axe sweep found + fixed a real pre-existing a11y bug** (`aria-prohibited-attr`, serious): every `EditableGrid` display cell (text/multiline/richtext) named its empty state with an `aria-label` on a role-less `<div>`; now carried in visually-hidden text (identical accessible name, no prohibited attribute, no role change — `role=button` was rejected as it reshaped the app-wide button landscape). **The frozen `onExitBoundary(dir)` seam + every EditableGrid seam DEFAULT-OFF held throughout: `e2e/d3-canvas.spec.ts` 8/8 green (the `?d3rf` canvas stays byte-identical).** Verification: `verify:fast` 1605 passed; architecture.spec.ts 6/6; d3-canvas 8/8; foundation/context-register/dimensions/parameters 13/13; screenshot-verified on the real `/architecture` route. **All acceptance criteria met (below).** Ready to archive → `done/`. **This unblocks 089 D3 graduation.** *(Known follow-up, out of scope: the read-only/viewer display branches of the same cells carry the same `aria-label` pattern — off the `/architecture` editor route; deferred to a viewer-mode a11y sweep.)* Prior hygiene retained: finding 6 (token-driven indent, `8ac3c72`), finding 7 (ref-based focus + guard).
...
- [x] **Empty state** — an empty Architecture surface shows a labeled add affordance plus one line of orienting guidance (finding 1). *(ArchitectureSurface empty-state test green.)*
- [x] **One add grammar** — a table is created via a single typed "type + Enter, clears, refocuses" path; no second differently-behaving "Add table" control remains (finding 2). *(Single-add-grammar test green; P1 removed the top standalone add-row, leaving one trailing phantom.)*
- [x] **Typed add-child** — adding a child opens an inline typed phantom, never inserts a literal `'New entry'` (finding 4). *(P3; D3-P3 inline-add-child tests green.)*
- [x] **Keyboard never dead-ends** — Tab/Enter carries from a table's last entry to the add-table point and into a new table's first entry, no mouse required (finding 3). *(P2 seam; P6 `architecture.spec.ts` keyboard-only flow proves the full chain end-to-end.)*
- [x] **(Direction 3)** — tables render on the shared `EditableGrid` stacked-per-table grammar threaded by one outer `EditableChainProvider`, byte-identical with entries and with 082's Design route; the `?d3rf` canvas consumer stays byte-identical (`d3-canvas.spec.ts` 8/8). *(P0–P2; P6 volume test proves it at 20×50.)*
- [x] **Token + a11y hygiene** — no raw pixel indent (finding 6); ref-based focus, role-aware (finding 7); listbox selection semantics (finding 8, P4: `role=listbox`/`option` + `aria-selected`). P6 axe sweep passes (no serious/critical) after fixing the empty-cell `aria-prohibited-attr`.
- [x] **Consistency** — the create control is out of the `t2-contextbar` navigate list; the jump list remains (finding 5). *(D1.)*
- [x] **No functional regression to 083** — 084 does not re-introduce or depend on the role-gate/fire-and-forget behavior; the add affordance's *presence* is 083's job. `verify:fast` green (1605 passed).
