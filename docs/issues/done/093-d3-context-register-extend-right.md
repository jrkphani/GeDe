...
- **Status**: **✅ SHIPPED (`dd8902b`, 2026-07-19) — rode 089-P2's decomposed Design core.** Removed the stranded "New context" button (register phantom row is the sole create; `c` still composes); the register EXTENDS RIGHT (canvas-only `.wc-node--design-register { width: max-content }` + `.register-scroll{overflow-x:visible}`, so proof columns no longer clip behind the frozen symbol column — far columns reached by panning; D2/normal route keep inner-scroll); LOD tuple-summary collapse below ~0.6 zoom via a `ContextRegister` `collapsed` prop driven by React Flow `useStore` (boolean selector — re-renders only on threshold crossing). **Scoping:** a pixel width-cap + the >8-column collapse trigger + focus-expand are DEFERRED to 089-P5's LOD pass (overview legibility here is the zoom-collapse); test 5 dropped per owner. Verified: d3-canvas 20/20 (+new 093 gate: offsetWidth>960, overflow-x visible, per-dim headers expand/collapse across the threshold, no button), flag-off design/coverage 8/8, screenshot-verified, xyflow stays out of the main bundle. **The forks (below) are resolved — kept for history.** D3-canvas UX refinement: the `?d3rf` Design node's context register clips horizontally (proof columns hidden behind the frozen symbol column) and the "New context" button is stranded above the ring. **Owner decisions:** fork 1 = **canvas-only**; fork 4 = **capped + LOD** (soft max width, columns collapse to a summary past the cap); fork 3 = **remove the button** — the register phantom row is the sole create affordance. See `### Fork resolutions (owner, 2026-07-19)` below. **Sequences after 089's Design-lane decomposition** (093 rides the graduation build — see 089's phased plan).
...
### Fork resolutions (owner, 2026-07-19)
- **Fork 1 (scope) → CANVAS-ONLY.** Gate "extend right / no inner scroll" behind the `?d3rf` D3 Design node. D2 lane page + normal single-Design route stay on the current `.register-scroll` inner-scroll (a later, separate decision). Test 8 (coverage toggle + child canvas unaffected) guards no leak.
- **Fork 4 (width) → CAPPED + LOD (Direction C).** A soft max node width; past it, per-dimension columns collapse into a tuple-summary column that expands near 1:1 / on focus. Protects overview-zoom legibility (owner explicitly chose the cap over unbounded).
- **Fork 3 ("New context" button) → REMOVE (Opt 1).** Delete the stranded top button (`DesignSurface.tsx:649-658`); the **register phantom row** (`ContextRegister.tsx:279-291`) becomes the sole create affordance — it is already the keyboard create-endpoint (the rail's last phantom Tabs into it, `DesignSurface.tsx:435-457`). **Note:** guided **compose-mode** create is NOT lost — the **`c` key** still calls `enterCompose()` (`DesignSurface.tsx:311-333`). The owner chose the simplest UI (no phantom→compose upgrade); the phantom keeps its justification-seed create (`ContextRegister.tsx:283-289`). Test 5 (one-create-grammar/compose) is therefore **NOT required** for this build — drop it.
- **Fork 2 (ring-vs-register layout) → resolver default: ring centered above the register, cap keeps the mismatch modest (Direction A within the capped width).** Owner did not call this explicitly; because fork 4 caps the width, the extreme 1500px-register whitespace case can't arise, so Direction A (ring centered above the capped register band) is the low-risk default. **Keep the ring's own measured shell a clean square** (hit-radius constraint, `base.css:1669-1672`). Revisit only if it reads wrong in the screenshot pass.
- **Fork 5 (LOD threshold) → resolver default: collapse per-dimension columns to a tuple-summary above ~8 dimension columns AND/OR below ~0.6 zoom; expand near 1:1 or on focus. No column virtualization** (register row count is modest; DOM width is fine). Tune the threshold against the screenshot/perf pass.

The original fork write-ups (kept for context):
