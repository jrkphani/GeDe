...
- **Status**: **✅ SHIPPED & LIVE — D1 + D2 + D3 GRADUATION COMPLETE (2026-07-20).** The D3 pan/zoom React Flow canvas is now the **DEFAULT** workspace in production (P7 `31b20a9`, live-verified on CloudFront: canvas renders at `/design` with no `?d3rf`, `role="main"` landmark, 4 lane nodes). All phases P0–P7 + 093 shipped (see the phased plan below, each marked SHIPPED). D1 (global rich-text toolbar) and D2 (three tiers as one lane page) shipped 2026-07-17. Follow-ups: the P3 satellite stub→live-core promotion and the canvas coverage/a11y items are filed as their own issues (see below). _(D3 was built behind the `?d3rf` dev flag P0–P6, then flipped to the capability-gated default at P7.)_ **084 D3 grid unification is now DONE (`done/084-...`), which was the gate — so the owner APPROVED graduating the canvas to the DEFAULT workspace AND building the 011 recursion + 012 coverage cluster now as edge-connected satellite nodes.** This **explicitly reverses** 085 ("canvas-as-visual, tables are the instrument") and SITEMAP §1/§2 ("page never scrolls") — a deliberate, owner-authorized product reversal. **Rich identifier columns: RESOLVED — keep PLAIN.** The authoritative build spec is **`## D3 GRADUATION — APPROVED BUILD PLAN (2026-07-19)`** below (phased, ≤5 files/phase, red-first; SAFE non-reversing work first, the default-flip last). **093 (context-register extend-right, canvas-only capped+LOD, remove the "New context" button) rides this — slot it in after the Design lane is decomposed.** See `## D1/D2 — SHIPPED` below for shipped context.
...
### Identifier columns — RESOLVED (owner, 2026-07-18): keep PLAIN
**Decision: `*.name` / `contexts.symbol` stay plain text** — no rich variant. This closes fork 4's remaining half. The hazards below are why. (Original analysis retained for context.)
...
**Build progress (behind the dev-only `?d3rf` flag; production unaffected):** **P0 SHIPPED** (`d1b5a3a`, pure `src/domain/laneLayout.ts`), **P1 SHIPPED** (`d1c3a86`, React Flow viewport + one real Design node), **P2 SHIPPED** (`63b64f4`, all three tier lanes as nodes; gate-c promote-popover-at-zoom + activeLane gating proven), **navigation SHIPPED** (`e53a138`, ⌘1/2/3 pan-to-lane via a module-level capture interceptor that keeps `?d3rf`, + focus-pan; also fixed a code-review HIGH — `useNodesState` seeds once, so node data was going stale across route changes). `@xyflow/react` 12.11.2 added; dagre deferred.
- **Reorder model — owner DECIDED (2026-07-18): decompose lanes into per-item nodes** (not whole-surface nodes). So the real P3 is the big one: each lane becomes N draggable item-nodes (e.g. Architecture = N table nodes) so a drag reorders `sort` on the canvas. The current whole-surface nodes are the interim harness.
- **P3 grammar-per-node spike — PASSED, GO (2026-07-18).** A throwaway spike decomposed the Architecture lane into per-table React Flow nodes hosting the REAL `EditableGrid`, driven at zoom 1.25. All five gates cleared: (a) in-node grammar survives per-node; (c) phantom-create-new-table + continue focus works (needs `requestAnimationFrame`×2 for the mount race); (d) constrained drag reorders `sort` with no scale math and no `{x,y}` persisted; (e) per-node heights are auto-**measured** (`node.measured.height`) and re-derive the stack (with a `fitView`-vs-measurement race to handle). **The make-or-break — (b) cross-node Tab — is SOLVABLE cleanly:** native Tab follows DOM/array order (breaks once a reorder desyncs it from `sort`), fixed by a capture-phase boundary handler that focuses the next-by-`sort` node **plus** a small additive `onExitBoundary(dir)` callback inside `EditableGrid` (the one seam — needed so a backward Shift+Tab out of an actively-editing first cell commits then relocates instead of the blur-race bouncing back).
- **P3 phased build plan** (production wraps the REAL `ArchitectureSurface` tables): **P3.0** — additive `EditableGrid.onExitBoundary(dir)` seam (fires where the grid already yields native Tab / `nextEditableCell` returns null; default undefined = no change for existing callers) — *the unblocker, do first*. **P3.1** — `laneLayout` N-nodes-per-lane (largely already handled by `computeLaneLayout`/`reorderWithinLane`). **P3.2** — Architecture lane as per-table nodes (`TableNodeAdapter` over the real `TablePanel`). **P3.3** — cross-node Tab via the P3.0 seam + reuse focus-pan. **P3.4** — constrained drag → the real tier-reorder `sort` mutation + `fitView` after `onNodesInitialized`. **Top risks:** (1) backward-Tab-out-of-edit without P3.0 (don't ship the capture-only hack); (2) N full `TablePanel`s (tree/promote/resolution popovers + dnd-kit + richtext) mounted + transformed at real data volume — perf/LOD unproven; (3) `fitView` vs async measurement (first-paint off-screen).
- **Open code-review follow-up (MEDIUM, not blocking):** `@xyflow/react`'s ~18.6 kB CSS still ships to prod (the JS is correctly tree-shaken out, but Vite never tree-shakes a side-effect CSS import even from a DCE'd module). The clean fix is to split the module-level ⌘ interceptor into a tiny eagerly-imported module (so it still beats AppShell) and `React.lazy` the heavy `WorkspaceCanvas` (React Flow + CSS) behind `d3CanvasEnabled()` — deferred because a naive lazy-load would break the interceptor's must-register-first ordering. Also add a build-size guard so the JS never silently re-enters prod. **Rich identifiers — owner DECIDED: keep `*.name`/`contexts.symbol` PLAIN** (fork 4 closed — no single-line-rich variant; avoids the tuple-hash + `InlineEdit` readOnly hazards).
...
## D3 GRADUATION — APPROVED BUILD PLAN (2026-07-19, owner)

> Code-grounded, owner-approved plan to **graduate the `?d3rf` pan/zoom canvas past the dev flag and make it the DEFAULT workspace**, and to **build the 011 recursion + 012 coverage cluster NOW as edge-connected satellite nodes**. This is the authoritative build spec; the exploratory D3 directions above are retained as context. It explicitly reverses two shipped decisions — 085 ("canvas-as-visual, tables are the instrument") and SITEMAP §1/§2 ("the page never scrolls") — named in the "SITEMAP + 085 lockstep edits" subsection below. It is the largest client-side surgery the app has attempted; sequencing is load-bearing.

### What graduation concretely means

Today the canvas is a DEV-only harness: `d3CanvasEnabled()` = `import.meta.env.DEV && ?d3rf` (`App.tsx:41-43`), so a production build folds the whole mount site away (`App.tsx:29-31,85-91` — `WorkspaceCanvas` is `React.lazy` behind `import.meta.env.DEV`) and `WorkspaceSurface` (the D2 lane page) always renders. **Graduation flips that default**: `WorkspaceCanvas` becomes the primary experience for the `project` / `tier` / `design` routes, React Flow (`@xyflow/react` JS + its ~18.6 KB stylesheet, ~88 KB gz all-in) now **ships to prod** on the lazy chunk (it was dead code before), and `WorkspaceSurface` is **retained as the narrow-screen (< 1024px) / reduced fallback** — the infinite canvas is desktop/tablet-first by necessity (089 cons; `SITEMAP.md:95` already concedes "< 400px read-mostly"). **The route grammar in `routes.ts` is PRESERVED throughout** (011/012/090 and most of the e2e suite depend on it — see the D2 correction). Tier, canvas depth (`contextPath`), and view (`DesignView`) stay parseable/serializable deep-links that map to a viewport/cluster target; the eventual REMOVAL of `contextPath`/`view` from the URL is **deferred past graduation as a separate simplification**, not part of this plan. The SITEMAP §1/§2 + 085 spec edits land **in lockstep with the final flip** (P7), per SITEMAP §6.

The build sequences **all safe, non-reversing work first** (thread the flag so it survives `navigate()`; decompose the Foundation + Design lanes into per-item nodes like Architecture already is; build the 011/012 satellites; LOD; the deploy-gate transition) and does the **actual default-flip LAST** (P7 — the one reversing step).
...
**P1 — ✅ SHIPPED (`d2f7fe7`, 2026-07-19) — decompose the Foundation lane into per-item nodes** (mirrors the Architecture decomposition).
- NEW `src/components/FoundationCanvasNodes.tsx` — a canvas-only adapter (`FoundationHeaderPanel` + `FoundationPropPanel`) reusing the tier1 store + `EditableGrid` + `RichTextEditor`, so `FoundationSurface` stays UNTOUCHED for its D2/normal-route consumers (mirrors how the arch column reuses the exported `TablePanel` rather than gutting `ArchitectureSurface`).
- `src/components/WorkspaceCanvas.tsx` — replaced the whole-surface Foundation `lane` node with a `foundationHeader` node (heading + Purpose + Existing-Scenario rich editors + the single add-prop phantom, id = `LANE_NODE_ID.foundation`, sort -1) + one `foundationItem` node per `tier1_props` value-prop (id = prop id, sort = prop.sort, degree on the handle). The canvas now owns the tier1 `load`; constrained node-drag pins x to the Foundation column (x=0) and on drop ranks siblings by center-y → **`reorderProp`** (Foundation reorders by RANK, not `sort`); cross-node Tab via the frozen `onExitBoundary` seam; `onPropCreated` continues focus into a new prop node. `LaneNode` narrowed to Design-only. `laneLayout.ts` needed NO change (already N-per-lane; the header's sort -1 sorts above items with no tie-break needed).
- `e2e/d3-canvas.spec.ts` (`@dev-flag`) — red-first: (1) header + per-prop nodes, Purpose + grid editable at zoom ≠ 1; (2) a value-prop node-drag reorders rank, x invariant holds, order survives reload (a real rank write, not node coords).
- **Verified:** `verify:fast` 1610 unit; `d3-canvas.spec.ts` 11/11 (+1 quarantine skip); **all three tier routes render loop-free on the canvas** (no "Maximum update depth" — the tier-route latent-bug watch held); screenshot-verified the decomposed column; prod build keeps xyflow + the new adapter OUT of the main bundle (dev-flag folds to false in prod).
- **Lesson:** a test focus-race — `onPropCreated`'s rAF×2 focus-into-new-node fires *after* the last phantom-add, so an immediate next interaction (typing into Purpose) got its focus stolen; a `waitForStableViewport` settle before the next interaction fixes it. Watch this in later phases whenever a create is followed by an unrelated edit.

**P2 — ✅ SHIPPED (`e0cc006`, 2026-07-19) — decompose the Design lane into a {register + ring} core: stack register OVER ring** (owner, 2026-07-16). *(the hard one — core risk)*
- **Key insight:** the flag-off `DesignSurface` and the flag-on canvas adapter are MUTUALLY-EXCLUSIVE render paths, so `DesignSurface` stays UNTOUCHED (flag-off zero-risk, mirrors P1's FoundationSurface-untouched approach). The two node bodies are SEPARATE React trees, so the shared compose draft moved to a store.
- NEW `src/store/canvasCompose.ts` — canvas-only Zustand store, the extraction of DesignSurface's compose machine (enter/exit/bind/unbind + clearIfMissing). Register `c`/phantom ENTERS; ring dots BIND/EXIT; selection stays shared via the contexts store.
- NEW `src/components/DesignCoreAdapter.tsx` — `DesignRegisterBody` (owns the ONE load effect + header + rail + register + the c/Esc/v/d/Tab handlers) + `DesignRingBody` (reads stores, renders Canvas/CoverageMatrix, owns hoveredMark + the vanished-draft cleanup) + a side-effect-free shared `useDesignCanvasContext` hook (canvasSelector/canvasReady).
- `src/components/WorkspaceCanvas.tsx` — `designRegister` (sort 0, keeps `LANE_NODE_ID.design` for ⌘3) + `designRing` (sort 1) node types/components replace the design `lane` node; `computeLaneLayout` stacks them by sort.
- `src/styles/base.css` — re-homed the `.design-surface-row > .editing-zone`/`.canvas-shell` declarations onto `.design-core-register .editing-zone` / `.design-core-ring` (the old two-flex-children selectors no longer match); `[data-suppress-canvas-empty]` is a generic attr selector, still works.
- `e2e/d3-canvas.spec.ts` (`@dev-flag`) — `.wc-node` count 3→4, `.wc-node--design` → `--design-register`/`--design-ring`; new P2 gate: register over ring, cross-node `c`-compose (register `c` → draft dot in the ring node), shared selection, at zoom ≠ 1.
- **085 boundary held:** ring = derived visual, register = authoring surface; only the fixed-viewport framing is gone.
- **⚠ Adversarial review (mandatory, store/write-path touch) caught + fixed TWO divergences pre-ship:** (1) CRITICAL — the extracted `enterCompose` dropped DesignSurface's inline `readOnly` guard, so a VIEWER's CoverageMatrix gap-click could create+bind a context → restored the guard on the ring's `handleComposeTuple` (the one path the register's c/button guards didn't cover). (2) HIGH — `hoveredMark` wasn't reset on canvas-nav; the ring node keeps a stable id so it never unmounts → a stale mark would mute the WHOLE child canvas on drill-in → added `setHoveredMark(null)` keyed on `canvasSelector` (mirrors DesignSurface:126-131). **Lesson: when splitting a component into two stable-id nodes, every per-navigation reset the original did on unmount/effect must be re-homed — the stable id means no unmount to rely on.**
- **Verified:** `verify:fast` 1610; `d3-canvas.spec.ts` 12/12 (+1 quarantine); flag-off design/design-layout/coverage/recursion 8/8; every tier route loop-free (zero console errors); screenshot-verified register-over-ring + cross-node compose; prod build keeps xyflow + adapter + compose store OUT of the main bundle.

> **093 rides HERE — slot it after P2.** `093-d3-context-register-extend-right.md` (extend-right, **canvas-only capped + LOD**, remove the top "New context" button → phantom-row create) sits on the decomposed Design core. **Its 5 forks are RESOLVED (owner, 2026-07-19 — see the 093 doc);** it runs as its own phased plan after P2 and reuses P5's LOD machinery.

**P3 — ✅ SHIPPED (`da87e86`, 2026-07-19) — recursion (011) as an edge-connected child-canvas satellite.** *(see cluster subsection)*
- **Scope call (owner, 2026-07-19): the child satellite is a SUMMARY STUB, not a second live `{register+ring}` core.** The `contexts` store is a hard Zustand singleton (one `canvasId`/`contexts`/`selection`, `store/contexts.ts:84-118`), so two live cores can't coexist without a central multi-canvas refactor — out of scope for a ≤5-file, safe, non-reversing phase. So "Open ▸" (register) / ring double-click now calls `useCanvasSatellitesStore.openSatellite(id)` (pure canvas state, **NO DB write** — the child canvas is only materialized on Enter), spawning a summary satellite node (parent symbol + child-context count) to the right of the core + a parent→child edge; pan-to-child; collapse unmounts; the parent core stays put. Deep entry still navigates via the satellite's **Enter ▸** (the existing drill), which resets the per-canvas satellite set. **→ Tracked follow-up: `## P3 FOLLOW-UP (deferred)` below — promote the stub to a live child core.**
- NEW `src/domain/clusterLayout.ts` — pure derived satellite placement + edge routing. **dagre/elk evaluated + DEFERRED:** the open-satellite set is a rightward tidy column (no crossing-minimization needed); elk is async (layout-race), dagre is a dep that buys nothing yet — the `computeSatelliteLayout` signature is the swap seam if P5 volume proves it. Satellites clear the register's **MEASURED** width (093 made it `width:max-content`, uncapped), not the nominal 960px (review-caught HIGH).
- NEW `src/store/canvasSatellites.ts` — canvas-only Zustand slice (open set + one-shot pan focus); the cross-tree channel between the drill (`DesignCoreAdapter`) and the renderer (`WorkspaceCanvas`, separate React trees — P2's cross-tree lesson). `WorkspaceCanvas.tsx` — satellite node type + hidden edge `Handle`s + derived `edges` (the app's FIRST React Flow edges; `nodesConnectable` stays false) + pan-to-child; `withDerivedPositions` splits lane vs satellite layout. `DesignCoreAdapter.tsx` — both `handleDrillIn` → `openSatellite` (flag-off `DesignSurface` untouched). `base.css` — satellite card + edge styles.
- **Gate MET:** `@dev-flag` e2e in `d3-canvas.spec.ts` (not `recursion.spec.ts` — the canvas gate lives with the other `?d3rf` specs) — "Open ▸" spawns a `.wc-satellite` + `.react-flow__edge`; pan-to-child; collapse hides; parent unchanged (`.wc-node` still 4); Enter ▸ deep-links (`/design/<ctx>`, grammar preserved) + resets satellites; **+ a wide-register no-overlap guard** for the review HIGH.
- **⚠ Adversarial review (mandatory, store touch) verified loop/state invariants clean AND caught a real HIGH:** satellite x was computed from the nominal 960px, but 093's uncapped register overlapped it at ~5+ dimensions → fixed to use the register's measured width (`withDerivedPositions`), `measuredSignature` now tracks width, + an e2e no-overlap guard. Also dropped 2 dead satellite data fields; `edges` now derive from reconciled `nodes` (never a dangling target). **Lesson: any satellite geometry that sits beside the Design core must key off the register's MEASURED width, because 093 deliberately uncapped it.**
- **Verified:** `verify:fast` 1624; `d3-canvas.spec.ts` 16/16 (+3 P3 gates, +1 quarantine skip); flag-off design/coverage/recursion 8/8; every tier route loop-free; PROD-FOLD holds (xyflow out of the main bundle); screenshot-verified the satellite + edge.

**P3 FOLLOW-UP (deferred) — promote the summary satellite to a LIVE child `{register+ring}` core.** *(owner asked this be tracked, 2026-07-19)*
- Today a satellite is a read-only summary (symbol + child count); authoring the child still requires Enter ▸ (navigate re-scopes the single core). To make an open child fully editable IN PLACE beside its parent (the spec's north-star "cluster"), the `contexts` store (and `dimensions`/`parameters`/`canvasCompose`) must become **per-canvas instances** (a store factory keyed by `canvasId`) instead of a singleton — a central refactor touching every store consumer. Sequence it as its own dedicated phase with its own budget + adversarial review; it composes with P5's lazy-mount/LOD (only on-path clusters mount live; deep collapsed children stay stubs). **Blocks 089 "done" only if the owner wants live child cores before graduation — otherwise it graduates as a post-089 issue.**

**P4 — ✅ SHIPPED (`4917219`, 2026-07-19) — coverage (012) as an edge-connected analytical twin.** *(see cluster subsection)*
- **FULLY LIVE (not a P3-style stub).** Key difference from P3: `CoverageMatrix` is read-only + fully derived and reads the SAME current-canvas stores the ring reads — no second canvas scope, so no multi-canvas refactor. `v` (and the header Coverage toggle) open a coverage TWIN node stacked BELOW the Design core (a design-lane node at `sort 2`, edge ring→twin) instead of the old route swap that REPLACED the ring; ring + coverage now COEXIST (the ring always renders the Canvas). A gap-cell click composes pre-filled (readOnly-guarded, issue 035) then pans back along the edge to the ring.
- NEW `src/store/canvasCoverage.ts` — a singleton BOOLEAN slice (`open` + one-shot pan `focus` + toggle/collapse/setOpen), not P3's id-keyed set (the twin is one-per-canvas). `WorkspaceCanvas.tsx` — `coverageTwin` node type (renders `DesignCoverageTwinBody`) + ring Bottom source `Handle` + the ring→twin edge; the core bodies' `view` is FORCED to `'canvas'` and a `?view=coverage` deep-link SEEDS the twin open (effect keyed on `[canvasIdentity, design.view]`). `DesignCoreAdapter.tsx` — `DesignRingBody` always renders `Canvas`; the coverage machine (CoverageMatrix + gap-compose) moved to a NEW `DesignCoverageTwinBody`; dropped the dead `navigate`/`preservedCanvasId`/`storeSelectedCanvasId`. **`routes.ts` UNTOUCHED** (grammar preserved for deep-link parity — the `view` param still round-trips; `v` is now spatial, mirroring how P3 kept the drill URL grammar).
- **Reuse (unchanged):** `CoverageMatrix.tsx`, `domain/coverage.ts`, `domain/gridWindow.ts`, `enterCompose(initialBindings)` (`canvasCompose.ts`). `clusterLayout.ts` NOT needed (the twin stacks via `computeLaneLayout` sort 2; the edge is inline).
- **Gate MET:** `@dev-flag` e2e in `d3-canvas.spec.ts` — `v` opens the twin + ring→twin edge (URL unchanged = not a route swap; ring still shows Canvas; twin is a 5th `.wc-node`), pan-to-twin, `v` again collapses; a gap cell composes pre-filled + pans back (`.canvas-dot-group--compose` in the ring); the `n / m documented` stat is live (`0 / 8` from the real 3×2 tuple space).
- **⚠ Adversarial review (mandatory) APPROVED, 0 CRITICAL/HIGH** — verified the seed-vs-toggle race (a `v` toggle is never clobbered — the seed effect deps don't change on toggle; `setOpen` is idempotent), loop-safety, and the readOnly guard. **Fixed the MEDIUM it caught:** satellites now clear the WIDEST measured design-column node (register, ring, OR the wide twin), not just the register, so a deep satellite stack never overlaps the twin. **Lesson (extends P3's): anything beside/below the Design core clears the whole column's widest MEASURED node.**
- **Verified:** `verify:fast` 1632; `d3-canvas.spec.ts` 18/18 (+3 P4 gates, +1 quarantine skip); flag-off design/coverage/recursion 8/8 (DesignSurface untouched); every tier route loop-free; PROD-FOLD holds; screenshot-verified register+ring+twin coexisting with a live stat.

**P5 — ✅ SHIPPED (`4ee3033`, 2026-07-19) — LOD summary cards + register width-cap + volume perf.** *(retires the top spike risk)*
- **Lane LOD:** `FoundationItemNode` + `ArchTableNode` swap their heavy real grid for a compact `.wc-lane-summary` card below `LANE_LOD_ZOOM` (0.35, deliberately under the register's 0.6 so small-project grammar specs at ~0.5 fit stay on real grids) — a body-local boolean `useStore` selector (093's pattern; re-renders only on a threshold crossing, never per frame, never through the reconcile hot path). **EDIT-SAFE:** a shared `useLaneLod` hook tracks focus in a REF (not state) so a cell click never re-renders mid-edit (which would cancel EditableGrid's click-to-edit) AND an actively-edited node WHEEL-zoomed out stays expanded (grid never unmounts → no dropped keystrokes).
- **Register 093-deferred bits:** the PIXEL WIDTH-CAP lands — `.wc-node--design-register` grows to content up to `max-width:1600px` then inner-scrolls (`overflow-x:auto`, was uncapped/visible), so a 20-dim register can't dominate the canvas or push P4's measured-width satellite/twin clearance unbounded. Guided compose force-expands (`registerCollapsed = zoomedOut && !composeContextId`). **The width-cap SUPERSEDES 093's deferred >8-col collapse + focus-expand** — the adversarial review showed a DOM-`.contains()` focus-expand breaks against the register's own PORTALLED combobox; the cap is a stable, edit-safe legibility bound that needs no focus tracking.
- **`onlyRenderVisibleElements` evaluated + REMOVED** (it unmounts off-screen nodes → breaks zoom-into-node reads; the real volume cost is register cell count, handled by the zoom-collapse + width-cap). `content-visibility` NOT used (conflicts with the measured-height stack). `clusterLayout.ts`/`laneLayout.ts` needed no change. Volume e2e uses an in-test FORMAT_VERSION-5 envelope (20 dims × 50 contexts) via the `input[type=file]` import path.
- **⚠ Adversarial review (mandatory, hot-path touch): NO loop bug; caught + FIXED 3 HIGH edit-awareness gaps** — (1) lane collapse dropping an in-flight edit → the `useLaneLod` focus-ref guard; (2) register `.contains()` focus-expand breaking against its portalled combobox → superseded by the width-cap; (3) compose not force-expanding a >8-dim register → `!composeContextId`. **Lesson: any zoom-driven LOD swap of an editable surface must be edit-aware (track focus, prefer a ref so it never re-renders mid-edit) — and DOM-`.contains()` focus checks are unreliable against portalled popovers.**
- **Gate MET / Verified:** `verify:fast` 1632; `d3-canvas.spec.ts` 21/21 (+3 P5 gates: volume overview→summary cards + zoom-in→grids, lane-edit-safety, compose-expand; +1 quarantine skip); flag-off design/coverage/foundation/architecture 14/14; PROD-FOLD holds; screenshot-verified lane summary cards at overview.

**P6 — ✅ SHIPPED (`e4a66b2` de-flake + `e81ab4e` un-tag, 2026-07-20) — deploy-gate transition: the `?d3rf` specs now RUN IN the gating `verify`.** *(the careful one — deploys never froze)*
- **P6.1 (`e4a66b2`) — de-flake, not black-box hardening.** The last `test.fixme` (focus-pan) could not be made e2e-deterministic (096: the `onFocusCapture` setCenter races a one-time post-`useNodesInitialized` fitView and no-ops under reduced motion, ~92% ceiling). Per the in-file note's own prescription, the pan-DECISION (pan-if-outside-margin + the screen point) was extracted to a pure `src/components/workspaceFocusPan.ts` and covered by 15 vitest cases (each edge pinned at the exact `<`/`>` boundary + 1px-past sibling); the flaky animation timing is left to React Flow; the fixme + note are deleted. Byte-equivalent to the inline logic (adversarially reviewed: APPROVE). The popover-anchor spec was already de-flaked at P5-era (reduced-motion + `waitForStableViewport` + geometry `expect.poll` — the template).
- **P6.2 — proof.** The 21-spec suite ran **22 consecutive green (20 local `e2e:dev-flag` runs + CI + a full-combined-suite run), 0 flakes**, while STILL `@dev-flag`. The animated-motion specs' `expect.poll`s converge reliably → **no project-wide `reducedMotion` change was needed** (avoided disturbing the flag-off suite — data-driven).
- **P6.3 (`e81ab4e`) — un-tag.** `package.json` `e2e` dropped `--grep-invert @dev-flag` → plain `playwright test` (so `verify` runs the canvas specs; full 78-block suite verified green under it); `.github/workflows/dev-canvas-e2e.yml` retired. **The `@dev-flag` tags are RETAINED as a one-line rollback lever** (re-add `--grep-invert @dev-flag` to instantly re-exclude if a canvas flake ever threatens to freeze deploys — the 096 failure mode). The RF bundle guard (`d3CanvasNav.guard.test.ts`) is **left as-is** — RF still folds out of the prod bundle; the budget-ceiling inversion is P7's, since RF only ships to prod at the flip.
- **Gate MET:** `verify:fast` 1647; full `npm run e2e` 78/78 green locally under the new command; CI `verify` (now INCLUDING the canvas specs) green + `deploy` fired; deploy pipeline never frozen.

**P7 — ✅ SHIPPED (`337157d` flip+bundle · `31b20a9` e2e+docs+review, 2026-07-20) — THE DEFAULT-FLIP + SITEMAP/085 lockstep, one PR.** *(the last, reversing, prod-behavior-changing step — adversarially reviewed)*
- **The flip is STORE-based** (the doc's `d3CanvasEnabled()`/`App.tsx:41-43` was already stale post-P0 — no such fn). `src/store/canvasMode.ts`: `canvasEnabled = canvasCapable() || d3rfInUrl()`, dropping the `import.meta.env.DEV` gate. `canvasCapable()` = `matchMedia('(min-width:1024px)') && !matchMedia('(prefers-reduced-data:reduce)')` (jsdom/SSR-safe → false → WorkspaceSurface). `?d3rf` RETAINED as a force-on override (narrow escape hatch + the d3-canvas e2e pin). `App.tsx:83-89` ternary unchanged (capability-based `canvasEnabled` already yields canvas-primary / WorkspaceSurface-`<1024px`-fallback); comments updated. Seed is once-at-boot — deliberately NOT re-seeded on resize (documented).
- **RF ships → PROD-FOLD inverted:** `vite.config.ts` dropped the `WorkspaceCanvas-*` `globIgnores` (precache-eligible); `d3CanvasNav.guard.test.ts` reframed to "RF budgeted to its lazy chunk" (assertions unchanged). Prod build verified: main bundle has NO xyflow; RF isolated to `WorkspaceCanvas-*.js` (~202 KB raw / ~88 KB gz).
- **e2e (the biggest piece):** a pre-flip sweep enumerated the breakage (31/57 fail); resolved by a **canvas/fallback SPLIT** — the ~57 non-`@dev-flag` specs re-pin to the shipping `WorkspaceSurface` fallback (`e2e/workspaceSurface.ts::forceWorkspaceSurface`, a `prefers-reduced-data` matchMedia shim), `d3-canvas.spec.ts` (22 `@dev-flag`, incl. a new canvas axe smoke) is the canvas suite. No assertion weakened (the `New context` button genuinely doesn't exist on the canvas — 093; canvas compose = `c`). `playwright.config.ts` gained `retries: CI ? 2 : 0` (canvas specs now gate alongside 57 others).
- **a11y:** added a `main` landmark (`role="main"` on `.workspace-canvas`) + a canvas axe smoke → **canvas is WCAG2 A/AA serious/critical clean.**
- **Adversarial review (MANDATORY) — all findings fixed:** 2 specs missed by the re-pin sweep pinned; the a11y gap closed (landmark + axe); resize-reseed tradeoff documented. Residual coverage/a11y/touch gaps + the cross-node-Tab flake filed as **issue 099** (non-blocking).
- **SITEMAP/085 (same PR):** SITEMAP §1-4 reframed (route grammar RETAINED as spatial deep-links; `<1024px` WorkspaceSurface fallback); 085 framing-supersession note (spirit reversed, no-on-ring-authoring rule HOLDS); the older 089 route-removal section marked superseded by this one.
- **Gate MET:** `verify:fast` 1650; full `npm run e2e` 78 pass + 1 retry-protected flake; canvas axe-clean; screenshot confirms canvas-default at 1600px (no `?d3rf`); CI `verify` + `deploy` green.

### The 011/012 cluster as edge-connected satellite nodes (dedicated)

Both features are today *route/view swaps* that fight the one-canvas model; both resolve to the **same spatial pattern — adjacent, edge-connected, on-demand satellite nodes** hanging off the Design {ring + register} core. Depth and view become **spatial state**, but — per "keep `routes.ts` intact" — the URL grammar still round-trips for deep-link parity; the canvas ADDS the spatial layer, it does not yet remove the URL state.

- **Recursion / drill-in (011) — P3.** *Today:* "Open ▸" calls `navigate({ kind:'design', contextPath:[...contextPath, id], view:'canvas' })` (`DesignSurface.tsx:493`) — a page-swap-in-place. *Graduated:* Open **spawns the child canvas as a new {ring+register} cluster node beside the parent, connected by a parent→child edge** (`clusterLayout.ts`, dagre/elk); pan-to-it; **collapse unmounts** (lazy). **Stores/surfaces reused unchanged:** `openChildCanvas` + `loadBreadcrumbs` (`DesignSurface.tsx:134,150`) + the DECIDED stale-parent-rebind reconciliation + `revertStaleRebind` Undo (`src/db/recursion.test.ts`, `src/store/recursion.test.ts`). The breadcrumb becomes a pan/collapse HUD. Position stays derived — the edge is the only meaning the arrangement carries. **e2e:** `recursion.spec.ts` gains a `@dev-flag` variant — drill spawns an adjacent cluster + edge; breadcrumb-jump pans; collapse hides; parent unchanged; `contextPath` deep-link resolves (`routes.ts:54-70`).
- **Coverage (012) — P4.** *Today:* `v` sets `view:'coverage'` via `navigate()` (`DesignSurface.tsx:567/:584`) and REPLACES the ring with `CoverageMatrix` (`:731`); a hollow-cell click enters `enterCompose(initialBindings)` pre-filled (`:216`). *Graduated:* coverage becomes the core's **analytical twin node, adjacent and edge-connected**, opened on demand — `v` = open/collapse the twin, not a route swap. A gap-cell click still composes — now a short pan back along the edge to the linked ring, pre-filled. **Stores/surfaces reused unchanged:** `domain/coverage.ts`, `domain/gridWindow.ts`, `CoverageMatrix.tsx`, `enterCompose`. The ~10k-cell matrix needs virtualization + LOD (P5): zoomed out = headline `n / m documented` + mini heat-strip; zoomed in = the full grid. **e2e:** `coverage.spec.ts` gains a `@dev-flag` variant — `v` spawns the twin + edge; gap-click pans + composes pre-filled; the stat increments.

### SITEMAP + 085 lockstep edits (§6: deviations are spec changes) — land WITH P7 in the same PR

- **`SITEMAP.md` §1 route map (`:9-25`)** — resolve the "Pending supersession" note (`:11`): the canvas is now the DEFAULT mount for `/p/:id`, `/p/:id/foundation|architecture`, `/p/:id/design[...]`; the **route grammar is RETAINED** as viewport/cluster deep-links (tier → pan-to-lane; `contextPath` → open-cluster; `view` → open-coverage-twin); deep links restore **viewport + focused cluster/node id** (and with 090 the root-canvas id).
- **§2 shell anatomy (`:32-34`)** — nuance "the page itself never scrolls": the single `.surface` scroll region is now a **pan/zoom infinite canvas** on desktop/tablet; narrow (< 1024px) falls back to the stacked-lane `WorkspaceSurface` (D2). Per-lane + per-node sticky headers stand; the shell `.context-bar` still hosts only the D1 `FormatStrip`.
- **§3 command palette (`:74`)** — "canvases (by lineage `α ▸ α2`)" resolves to **pan-to-cluster**, not a route push; add "jump to lane" and (090) "jump to canvas".
- **§4 keyboard map (`:79-89`)** — ⌘1/2/3 = **pan/zoom-to-lane** (already the interceptor, `d3CanvasNav.ts:80-94`); `v` = **open/collapse the coverage twin**; `c` = compose in the Design core; drill = spawn child cluster.
- **`085` Status / Supersession note** — the workspace-canvas framing **reverses 085's "canvas is a glance, tables are the instrument" *spirit*** (the canvas is now the primary organizing surface), but 085's **narrower rule still HOLDS**: no on-ring authoring; the ring stays a derived visual and the register is the authoring surface inside the Design core (P2). A *framing* reversal, not a re-opening of 082 Phase-2 on-ring authoring. (Fork 7 / fork 10 resolve at their recommended defaults: 085 holds inside the lane; pan/zoom is meaning-free viewport, positions derived.)

### P7 e2e breakage enumeration (SWEPT 2026-07-20 — throwaway worktree, canvas force-flipped default)

Ran `playwright test --grep-invert @dev-flag` (the 57 non-canvas specs) with `canvasEnabled` forced `true`. **Result: 31 FAIL / 26 PASS.** The 31 collapse into **THREE fixes, not 31**. Root mechanism: fitView shrinks the workspace to **~0.38 scale** and translates it; register/grid/table content all render in the DOM but deep affordances land OUTSIDE the 1280×720 viewport, and Playwright's native `scrollIntoViewIfNeeded` cannot move the React Flow pane (verbatim: "element is outside of the viewport").

- **Category B — fitView-transform reach (20 tests, THE big lever).** Every click/focus on a deep affordance (`New context`, `.editable-grid` α-row cells, `Promote`, `Rename Canvas 1`, coverage cell) times out at 30s. **Fix = ONE pan/zoom-to-node test seam** the specs call before interacting (e.g. `Meta+3` design-lane framer already exists, or a `setCenter/fitView({nodes:[target]})` seam zooming the node to ~1.0 + centering). Clears ~20 at once. Files: canvas-compose (56,111), canvas-focus (82,127,155,162 — ALL 4), canvas-spline (93), canvas-selection (80,109 — BOTH), canvas.spec (69), context-register (64,121,164 — ALL 3), design-layout (185), export-import (99), recursion (69,124 — BOTH), architecture (48), canvas-switcher (37), coverage (59). Alternative: re-pin the register-authoring suite as `<1024px` narrow smokes that keep mounting `WorkspaceSurface`.
- **Category A — hard selector breaks (2 tests, rewrite).** `workspace.spec.ts:8` (`.workspace__lane--foundation` gone → use `.wc-node--foundation`/`--architecture`/`--design-register`); `shell.spec.ts:33` (`[data-view="coverage"]` absent on canvas → assert `.wc-node--coverage-twin` or wire `data-view` onto the ring/twin node).
- **Category C — assertion/geometry/axe/keyboard (9 tests, per-test).** Geometry with hard-coded WorkspaceSurface stacking coords (design-layout 67,110 — meaningless under transform); `.canvas-shell` label-tier by measured width differs in a 0.38× node (canvas-parameters 77, canvas.spec 103); keyboard focus-threading across sibling table nodes + quiet-hint-on-focus (architecture 240,361,329); axe `include` scope resolves to nothing (architecture 290 → retarget to canvas node); foundation rank "1°" not rendering in Foundation item nodes (foundation 32 — verify Foundation decomposition preserves rank display).
- **26 PASS unchanged (good P7 scope news):** shell/nav/tabs/theme/not-found, ⌘K palette (incl. select-context-on-canvas), projects CRUD, dimensions+parameters rail authoring, foundation prose + round-trip, undo-redo, sync-status, the `c`-key compose and `v`-key coverage toggle, canvas-native empty-state + param-dot-label assertions.
- **⚠ ENV CAVEAT for P7 runs:** the sweep's isolated worktree had NO local `node_modules` (deps hoist to parent), so Vite `fs.allow` 403'd `pglite.wasm`+fonts → DB never booted → ALL 57 falsely failed until a throwaway `server.fs.allow:['<repo>']` was added. **P7 must run where `node_modules` resolves inside an allowed path** (i.e. on `main` / a normal branch, not a bare worktree) or add `server.fs.allow`.
- **Recommended P7 sequencing:** pan/zoom-to-node seam FIRST (clears ~20 B) → 2 A rewrites → triage 9 C's individually.

### Top risks + mitigations

1. **Deploy-gate / `@dev-flag` transition (P6) — the sharpest operational risk.** Moving the `?d3rf` specs into the gating `verify` re-creates the exact 096 failure (one flaky canvas test froze ~8 prod deploys). **Mitigation:** de-flake the two `test.fixme` viewport specs FIRST and prove green across N runs while STILL `@dev-flag`; only then remove tags + drop `--grep-invert`; keep a one-line rollback (re-add `@dev-flag`). Do P6 immediately before P7.
2. **Default-flip blast radius on the whole e2e suite (P7).** ~50+ non-`@dev-flag` specs assume the `WorkspaceSurface` DOM (native scroll, `.workspace__lane--*`); the canvas replaces that with `.react-flow__node`, a transformed viewport, and no native `scrollIntoView`. **Mitigation:** flip is LAST; run the full suite against the canvas in a branch to enumerate breakage before merge; use the **D2 P5 Rule-12 sweep as the template**; keep `WorkspaceSurface` as the narrow fallback so narrow specs pin it.
3. **Perf / LOD at volume (P5).** Three real surfaces + N Architecture nodes + satellites mounted and transformed is the headline cost. **Mitigation:** lane-summary LOD, lazy-mount satellites, `content-visibility: auto`, `@tanstack/react-virtual` past ~10k rows; retire with the volume e2e (084's `.gede.json` 20×50 seed).
4. **093 rides on this graduation** — slot after P2; its forks are already resolved (093 doc).
5. **`?d3rf` flag threading (P0) is a hard prerequisite** — every satellite interaction navigates; if the flag doesn't survive `navigate()` the canvas drops mid-build. Move to `canvasMode.ts` before P3/P4.
6. **Focus-pan ergonomics at bulk entry** — the pan-if-outside-margin heuristic (`WorkspaceCanvas.tsx:510-539`, `FOCUS_PAN_MARGIN`) needs real-user tuning so the viewport never fights the typist (north star).
