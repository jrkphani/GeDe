...
- **Status**: ✅ SHIPPED & ARCHIVED (2026-07-21). The discoverability bundle landed, verified, and DEPLOYED (CI verify + deploy green on `585f872`). The append-symmetry item is a **recorded deferred owner-fork** (a decided non-goal for consistency, not pending work — reopen if the owner wants Enter-append). Written from an adversarial UX debate (proponent vs. skeptic, both grounded in the code).
