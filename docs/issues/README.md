...
| [062](062-invitee-cannot-discover-pending-invitation.md) | Bug (#8 real blocker): a fresh invitee never receives the invitation — `invitations` isn't synced/streamed to their device, so 060's badge never appears + nothing to accept. Needs a server delivery path (dedicated `/invitations/mine` endpoint, or email-scoped read-path) | M9 | 056, 057, 058, 060 |
| [063](063-no-protected-routes-signed-out-access.md) | Bug: no protected routes — signed-out users still access project data in the same browser (local-first data isn't gated/cleared on sign-out). Needs a product decision (clear-on-sign-out vs. gate cloud content vs. full auth-gate) | M9/M10 | 033 |
