...
| [107](done/107-transaction-wrap-multi-step-mutations.md) ✅ | Transaction-wrap ALL ~21 multi-step DB mutations for atomicity — P1 `moveTier2Entry` (`dc51894`) · P2 tier2 subtree/promote (`52955a5`) · P3 reorder family (`c12b894`) · P4 cascades (`b57fb8e`) · P5 binding/param incl. `deleteParametersUnbinding→removeParameter` composition + `createProject` hoist + `openChildCanvas` (`8126537`). RED-first rollback tests per mutation; DB-reviewer APPROVE each phase; store layer unchanged | M8/M11 | — |
