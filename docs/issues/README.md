...
| [099](099-canvas-default-coverage-and-a11y-followups.md) **OPEN** — non-blocking remainder | Canvas-default coverage/a11y follow-ups (089-P7). DONE: canvas-side hover-mute + dual-empty-state e2e SHIPPED (`d3-canvas.spec.ts:1609`/`:1570`, commit 8cc03d2); cross-node-Tab flake retry-mitigated + axe smoke; label-tier non-issue; 2b/2c shipped (CoverageMatrix valid ARIA grid; dot hit target in screen space). REMAINING (all non-blocking): touch/tablet pan-zoom + node-drag on a real coarse-pointer device (manual; no automated coverage); broader canvas axe extension (Foundation/Architecture lanes + keyboard focus-order); optional label-tier-stable-across-zoom lock (LOW); deterministic focus-settle for the cross-node Tab spec | M7 | 089-P7 |
...
| [105](done/105-architecture-tree-promote-demote-keyboard.md) ✅ | Architecture tree keyboard tree-building (P0–P5 + both LOW nits): **P0** kill sub-child (Tab intercept) · **P1** Enter=sibling · **P2** ⌘]/⌘[ promote/demote · **P3** ⌥⇧↑/↓ move · **P4** tree ARIA + KeyHints · **P5** ⋯ row-action menu (pointer twin of the chords via shared target-computers; disabled = keyboard no-op). Sibling logic deduped into `domain/entryTree`; chords exclude `ctrlKey`. Residual (non-blocking, tracked in-row): systemic PGlite-txn-wrapping of multi-step mutations like `moveTier2Entry`; minor P1 phantom-anchor polish (phantom stays after series-start row, not newest sibling) | M7 | 084, 102, 104 |
| [106](106-canvas-live-child-core-refinements.md) **OPEN** — non-blocking (from 100) | Canvas live-child-core refinements: zoom-LOD auto-culling of off-screen/deep child cores; nested-drill grandchild positioning; presence + palette reach child-core selections. None crash/corrupt | M7 | 100 |
