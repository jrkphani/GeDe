...
**P7 — THE DEFAULT-FLIP (LAST — the one reversing step) + SITEMAP/085 lockstep.**
- `src/App.tsx` — `d3CanvasEnabled()` becomes a **capability check** (default ON: desktop/tablet width + not reduced-data), no longer `import.meta.env.DEV && ?d3rf`; the canvas is the primary mount for `project`/`tier`/`design`; `WorkspaceSurface` becomes the < 1024px narrow fallback.
- `src/store/canvasMode.ts` — seed from capability, not `?d3rf`. `docs/SITEMAP.md` — §1/§2/§3/§4 lockstep edits (below). `docs/issues/done/085-*.md` — a Status/Supersession note (below). `e2e/**` — the full non-`@dev-flag` suite scoped to the now-default canvas (**the D2 P5 Rule-12 sweep is the template**); narrow-viewport specs pin `WorkspaceSurface`.
- **Gate:** the full `verify` (all e2e, canvas-default) green; the SITEMAP + 085 edits merged **in the SAME PR** (SITEMAP §6); a narrow-viewport smoke still renders `WorkspaceSurface`.
