...
**P4 — coverage (012) as an edge-connected analytical twin.** *(see cluster subsection)*
- `src/components/WorkspaceCanvas.tsx` — `v` opens/collapses a coverage twin node edge-connected to the Design core (was the route-swap `DesignSurface.tsx:567/:584/:731`); a gap-cell click pans back along the edge and enters compose pre-filled.
- `src/domain/clusterLayout.ts` — twin placement + edge. `e2e/coverage.spec.ts` (add a `@dev-flag` canvas variant).
- **Reuse:** `CoverageMatrix.tsx`, `domain/coverage.ts`, `domain/gridWindow.ts`, `enterCompose(initialBindings)` (`DesignSurface.tsx:216`).
- **Gate:** `@dev-flag` e2e — `v` toggles the twin (not a route swap); gap-click composes pre-filled; the `n / m documented` stat is live.
