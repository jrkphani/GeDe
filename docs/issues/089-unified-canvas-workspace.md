...
**P5 — LOD + lazy-mount satellites + perf at volume.** *(retires the top spike risk)*
- `src/components/WorkspaceCanvas.tsx` — lane-summary LOD (mount the real grid only near 1:1; a summary card zoomed out; virtualization by *lane*, not row); lazy-mount clusters (only on/near the open path; a collapsed deep child is a single stub); coverage twin = headline stat + mini heat-strip zoomed out, full grid zoomed in.
- `src/domain/laneLayout.ts` / `clusterLayout.ts` — LOD thresholds. `src/styles/base.css` — `content-visibility: auto` off-screen + LOD styles. `e2e/d3-canvas.spec.ts` (`@dev-flag`) — reuse 084's `.gede.json` bulk-import for a 20×50 volume seed.
- **Gate:** `@dev-flag` e2e — overview renders summary cards (not full grids) below the zoom threshold; the volume import stays interactive (deep cell-open < 3s, 084's bar); no console errors.
