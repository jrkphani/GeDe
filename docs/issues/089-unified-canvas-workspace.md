...
**P2 — decompose the Design lane into a {ring + register} core: stack register OVER ring** (owner, 2026-07-16). *(the hard one — core risk)*
- `src/components/WorkspaceCanvas.tsx` — replace the whole-surface Design `lane` node (`:207-214,408-421`) with a register node (rail + `ContextRegister`) stacked over a ring node (`Canvas`). `contextPath`/`view`/`canvasId` (already threaded, `:310-315`) still drive it.
- NEW `src/components/DesignCoreAdapter.tsx` — a wrapper mounting the register-only / ring-only pieces (cleaner than surgery on the ~700-LOC `DesignSurface`).
- `src/styles/base.css` — reconcile with the ring-on-top relayout (`:1614-1625,1673-1681` per 093). `WorkspaceCanvas.test.tsx` — red. `e2e/d3-canvas.spec.ts` (`@dev-flag`).
- **085 boundary:** the ring stays a **derived visual glance**, the register stays the **authoring surface** — 085's "no on-ring authoring" rule still holds inside the core; only 085's *fixed-viewport* framing is gone (see lockstep edits).
- **Gate:** `@dev-flag` e2e — register cell edit + `c` compose + ring dot select at zoom ≠ 1; `DesignView`/`contextPath` still round-trip via `routes.ts`.
