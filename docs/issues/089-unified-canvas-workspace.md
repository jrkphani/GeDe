...
- `src/domain/laneLayout.ts` — already N-nodes-per-lane; add the Foundation tie-break (the parked `laneLayout` follow-up).
- `src/components/WorkspaceCanvas.test.tsx` — red. `e2e/d3-canvas.spec.ts` (`@dev-flag`).
- **Note:** Foundation reorders by **`rank`** (dnd-kit re-rank `FoundationSurface.tsx:28-51,200-213`), not `sort` — the constrained node-drag must hit the Foundation rank mutation (parallel to Architecture's `reorderTable`, `WorkspaceCanvas.tsx:566-586`).
- **Gate:** `@dev-flag` e2e — Purpose + each value-prop edit/Tab at zoom ≠ 1; a prop node-drag reorders `rank`, no `{x,y}` persisted.
