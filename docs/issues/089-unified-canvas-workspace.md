...
- **Reorder model — owner DECIDED (2026-07-18): decompose lanes into per-item nodes** (not whole-surface nodes). So the real P3 is the big one: each lane becomes N draggable item-nodes (e.g. Architecture = N table nodes) so a drag reorders `sort` on the canvas. This RE-OPENS how `EditableGrid`'s keyboard grammar works when each table is its own node (the spike only proved a whole-surface node) — treat P3 as its own spike-then-build. The current whole-surface nodes are the interim harness.
