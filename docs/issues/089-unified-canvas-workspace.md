...
**P3 — recursion (011) as an edge-connected child-canvas satellite.** *(see cluster subsection)*
- `src/components/WorkspaceCanvas.tsx` — drill-in spawns a child {ring+register} cluster node beside the parent + a parent→child edge; pan-to-child; collapse unmounts.
- NEW `src/domain/clusterLayout.ts` — derived placement of a child cluster to the RIGHT of its parent + edge routing (**dagre/elk earns its place HERE** — spike finding "defer dagre to the 011/012 clusters").
- reuse `useContextsStore.openChildCanvas` / `loadBreadcrumbs` / stale-rebind (`DesignSurface.tsx:134,150`; `src/db/recursion.test.ts`, `src/store/recursion.test.ts`). `e2e/recursion.spec.ts` (add a `@dev-flag` canvas variant).
- **Gate:** `@dev-flag` e2e — "Open ▸" spawns a child cluster + edge; pan-to-child; collapse hides; parent unchanged; the `contextPath` deep-link (`routes.ts:54-70`) still resolves.
