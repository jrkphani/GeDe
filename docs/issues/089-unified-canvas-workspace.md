...
**P6 — deploy-gate transition: de-flake the `?d3rf` specs, THEN move them into the gating `verify`.** *(the careful one — do NOT freeze deploys)*
- `e2e/d3-canvas.spec.ts` — **first**, de-flake + re-enable the 096 `test.fixme` quarantined tests (popover-anchor `:156`, focus-pan `:286`) with deterministic pan-settle / zoom-invariant asserts, and **prove green across N consecutive runs while STILL `@dev-flag`** (non-gating).
- **Then only** (once green is proven): remove the `@dev-flag` tags from `d3-canvas.spec.ts` + the P3/P4 canvas variants; `package.json` — drop `--grep-invert @dev-flag` from the `e2e` script (`:14`); retire `.github/workflows/dev-canvas-e2e.yml` (its own `:19-22` instruction); invert the parked React-Flow zero-size guard into a budget CEILING (RF now ships ~88 KB — allowed).
- **Gate:** the gating `verify` (now INCLUDING the canvas specs) green across N consecutive runs; the deploy pipeline not frozen (`gh run list --json`); bundle within the new budget.
