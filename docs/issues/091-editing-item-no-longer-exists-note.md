...
- **Status**: PARTIAL — the **heal-origin suppression shipped** (`4641a75`, 2026-07-18): D1 heal-on-load background writes are tagged `origin:'heal'` (`richTextConvert.ts`) and their `unknown_entity` rejection no longer surfaces the note (`sync.ts flush()` filters them); user-initiated rejections still surface (no masking). This closes **hypothesis 2** (the prime suspect). **General root-cause: INSTRUMENTED, awaiting a repro.** The handler previously only *returned* the rejection to the client and never logged it (a 7-day CloudWatch filter on `unknown_entity` returned zero — the string was never logged). Diagnostic logging is now deployed (`878eb97`): the `!tenancy.ok` branch logs `[writeApi][091] tenancy rejection {reason,table,op,entityId,mutationId,declaredWorkspaceId}`. **Next: owner reproduces the note on the live build (a USER edit, not a heal), then read `/aws/lambda/Gede-Test-Api-WriteApiFunction5106E371-2PvLQCdOFbzl`** (`aws logs filter-log-events --filter-pattern "091" … --profile phani-quadnomics`) for the exact mutation; cross-reference the `entityId` against the row's DB state (debug API) — a tombstoned row ⇒ hyp. 1 (stale-delete echo), an absent-never-inserted row ⇒ hyp. 3 (edit-before-insert race). Remove the log once root-caused. Intermittent, cosmetic (self-heals to `Synced`).
