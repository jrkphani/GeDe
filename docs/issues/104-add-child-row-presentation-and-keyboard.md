...
- **Status**: 🟡 CORE SHIPPED (2026-07-21) — all three facets implemented, verified (tsc/eslint/stylelint clean; vitest 1665; full e2e 90/90 incl. 102 preserved; adversarially reviewed → SHIP). A few LOW polish items remain (see "Remainder"). Surfaced immediately after 102 shipped (the child row appears; its presentation was the problem).
  ...
  - **Remainder (LOW, from adversarial review):** (1) `dismissOnBlur={false}` means clicking **empty space** (not a cell) leaves the phantom armed + still suppressing editing until Esc / a cell click — recoverable, but a wart; consider an outside-pointerdown dismiss if it bites on the canvas. (2) Add tests for: Escape still dismisses with `dismissOnBlur=false`; a plain-text (non-richtext) cell click while armed dismisses+edits; Shift+Tab lands the first cell; the empty-space-leaves-armed behavior. (3) The rAF-after-unmount focus handoff is correct under React 18 discrete events but is an implicit timing invariant worth a guard/comment.
