...
- **Status**: 🟢 **P0–P4 SHIPPED (2026-07-22); P5 held for owner go.** P4 (tree ARIA + KeyHint discoverability) added a SR-only `role="tree"` of `role="treeitem"`s (`aria-level`/`aria-expanded`, off the `<tr>` to stay axe-clean; promote `role="option"` listbox preserved) + aria-hidden `⏎`/`⌘]`/`⌘[` KeyHint chips (gated by `showKeyHints`, off on canvas). Fully verified locally (impl run): tsc + eslint (0 err) + stylelint clean, vitest 1681, **full e2e 103 passed with ALL axe scans green** (architecture P6 + the new 105-P4 role=tree + all three d3-canvas `@dev-flag` scans + coverage matrix), stress 10/10. (One `d3-canvas.spec.ts:179` `[data-db-ready]` timeout is a pre-existing PGlite DB-init flake, unrelated, passes in isolation, absorbed by CI retries:2.) `EditableGrid` is byte-identical — nothing shared touched. P0–P3 below. Keyboard tree-building is fully in place: **P0** kill the sub-child Tab-fallthrough; **P1** Enter = new-sibling series (type-to-create phantom, Architecture-scoped opt-in seams so Design/Foundation are byte-identical; 102/104 preserved); **P2** `⌘]`/`⌘[` promote/demote via a `moveEntry` store action over the tested `moveTier2Entry` (one undo step/gesture, sync-enqueue complete, focus-follows); **P3** `⌥⇧↑/↓` move among siblings. Adversarially reviewed across P0/P1 (2 rounds → 5 HIGH found + fixed) and P2/P3 (APPROVE, 0 crit/high; one MEDIUM `e.repeat` held-chord guard fixed; 2 LOW nits deferred). Verified: tsc+eslint clean, `verify:fast` 1678, full e2e 102, unit+e2e for every path, screenshots. **Follow-ups (P4/P5, not in the approved scope):** P4 tree ARIA (`aria-level`/`aria-expanded`) + `KeyHint` chips teaching `⏎`=sibling/`⌘]`=child; P5 the `⋯` row-action gutter menu (single-row commands) — see the IA section. Plus 2 LOW review nits (dedupe `siblingGroup`/`siblingsOfIn`; exclude `ctrlKey`). Kept OPEN for those.
