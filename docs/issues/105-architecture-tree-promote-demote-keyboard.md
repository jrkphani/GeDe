...
- **Status**: IN PROGRESS — **P0 + P1 SHIPPED (2026-07-22)**; P2+P3 next. Owner decisions: build P0–P3; promote/demote = `⌘]`/`⌘[`. **P0 (Tab-fallthrough sub-child fix) + P1 (Enter=new-sibling series) done** — sibling creation is a type-to-create phantom (unified with add-child), Architecture-scoped opt-in seams (`richTextTabAdvances` + `onEnterCreateSibling`) so Design/Foundation Enter/Tab are byte-identical; 102/104 preserved. Two adversarial-review rounds (round 1 found 4 HIGH regressions — orphan rows, double-Enter race, phantom hijack, non-opt-in Tab — all fixed; round 2 found 1 more HIGH — bidirectional exclusivity — fixed). Verified: tsc+eslint clean, `verify:fast` 1675, full e2e 99, unit+e2e regression tests for every finding, screenshot-confirmed. Next: P2 (`⌘]`/`⌘[` promote/demote via `moveEntry`) + P3 (`⌥⇧↑/↓` move). The Tier-2 parent/child editing was clunky: no keyboard sibling-vs-child, no promote/demote, an accidental sub-child bug, row *controls* in data cells.
