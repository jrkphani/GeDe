...
- **`099` (`docs/issues/099-canvas-default-coverage-and-a11y-followups.md`)** — PARTLY DONE (`76ebf08`: cross-node-Tab flake hardened; canvas axe smoke extended to a populated register; label-tier-vs-zoom closed as a non-issue). **The concrete REMAINING items:**
  - **2b (CRITICAL a11y):** `CoverageMatrix.tsx:388/441` renders absolutely-positioned `role="gridcell"` with no `role="row"` parent → invalid ARIA grid on BOTH surfaces (surfaced by the new canvas axe scan; the coverage-twin scan is currently NOT re-enabled, awaiting this). Fix with `display:contents` `role="row"` wrappers; then re-enable the twin axe scan in `d3-canvas.spec.ts`. Verify on both surfaces.
  - **2c:** `Canvas.tsx:130` hit-radius is transform-blind — the ~44px dot target shrinks under canvas zoom (WCAG 2.5.5). Feed a screen-space width into the hit-radius calc only.
