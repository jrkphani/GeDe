...
**Status:** accepted, 2026-09-13 — amended by ADR-039 (a commit into a compact row wraps it)
...
## ADR-039 A formula or reference committed into a compact row wraps the row (amends ADR-024)

**Status:** accepted, 2026-09-13 — FX-07, REF-01, GRID-09 (`final-fix/document-shell`, closes #142)

**Context.** ADR-024 ruled that the expression line FX-07 requires (and the path REF-01 requires) renders only in a wrapped row, with the compact row carrying the expression as a tooltip, because a second line cannot be drawn in a 22 px row without changing its height, which is a document edit. The audit (#142) filed the gap: the PRD rows do not qualify the secondary line. Two ways were open: draw the line inside the compact row, or make the row two units when a formula lands in it. The compact cell's content box is 21 px; the cell line is 11.5 px at 1.35 and the label step (9 px) is the type scale's floor (#65), so two legible lines need at least 27 px — and the DS makes 1.7 the line height for Indic text. The line cannot be drawn.

**Decision.** Committing a formula or a reference into a row that is compact wraps that row in the same transaction (`GridCommands.commitCell` and the column-scope `fillColumn`), so the wrap is part of the same undo step and the expression line is visible from the first paint. The engine's own cells — a derived column's, a pulled row's — are not committed by a person and do not wrap a row: a derived column shows its pipeline in the lineage header (ADR-032), a pulled cell shows its provenance badge. A row is never unwrapped by a commit: once wrapped its height is the person's, and the Table tab's "Wrap every row" or the cell menu's Wrap text unwrap it (#128).

**Consequences.** Addresses below a formula move down one when its row wraps — the same lattice write a divider or the wrap switch makes (non-negotiable 3), announced through the commit. The hierarchy panel names a formula row by its projected expression (`=Sum(B5:B6)`), never its id tokens (PRD §20). ADR-024's tooltip stays for a row someone has since unwrapped. The formula e2e journeys that wrapped a row to assert the line now find it wrapped by the commit.
