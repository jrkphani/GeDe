...
- **⇧⌘→ / ⇧⌘← move focus to the next / previous object on the sheet** — a table (its tab stop: the selected cell, else its first), a graph (its header) — in render order (the sheet's tables, then its graphs — the document's list, not the DOM's, because the canvas culls tables outside the viewport), wrapping, announced by the object's name; the object is revealed (`revealBounds`, as FIND-07 does) before its entry takes focus. Bound in the shell from anywhere in the document except an open editor and any text field: ⇧⌘→ / ⇧⌘← are the platform's select-to-line-end in a field, so `useShortcuts` skips editable targets and the binding is off while a cell is edited. The graph's own arrow handlers ignore modified arrows so the chord passes them. Not ⌃⌥→: off Apple platforms that is the same keys as ⌥⌘→ (add column). Listed on the sheet under View with an `extra: 'ADR-042'` marker; `shortcut-map.test.ts` pins it beside ADR-030's and ADR-033's rows. GRID-05 is untouched: Tab past the last cell still appends a row.
...
## ADR-044 Every mail is one token-derived layout in the recipient's locale; Cognito's codes go through a custom-message trigger, attached only once the pool sends through SES

**Status:** accepted, 2026-09-13 — AUTH-03, AUTH-04, I18N-05, SHARE-02 (`feat/branded-mail`)

**Context.** The pool sent its stock templates ("Verify your new account" / "The verification code to your new account is 737148"): unbranded, English only, from `no-reply@verificationemail.com`. The share mail (#76, #152) had its own hand-written HTML in `services/sync`, English only, with no relation to the design system. The product ships six locales (I18N-05) and the design system asks for one voice, one type scale and a palette that never leaves `packages/tokens`; an email client reads none of that (no custom properties, no `color-mix()`, no web fonts, `<style>` often dropped, data-URI images blocked by Gmail). Cognito offers three ways in: the pool's message templates (static, one language, and — for the passwordless EMAIL_OTP code — an L1-only `EmailAuthenticationMessage` the L2 does not expose), a custom-message trigger (the same templates rendered per event, with the user's attributes in hand, but a trigger that throws refuses the sign-in itself), and a custom-email-sender trigger (a Lambda that decrypts the code with KMS and sends the mail itself — the sender becomes ours, and so does every delivery failure). The web app never wrote the pool's `locale` attribute; the account's choice lived in `users.locale` alone, which Cognito cannot see when it sends a sign-in code before the service is asked anything.

**Decision.**

- **One package, `@gede/mail`, renders every mail**: framework- and dependency-free, so the same module is bundled into the sync image (SES) and into the Cognito trigger (Lambda). One layout — 600 px table markup, inline styles, a `<style>` block carrying only the dark swap under `prefers-color-scheme` plus the `color-scheme` meta, the mark as the hosted `icon-192.png`, body copy at 16 px, the live amber on the code and nowhere else, one `<a>` at most, a footer that says why the mail arrived. **The palette is generated from `tokens.css`** (light from `:root`, dark from the `[data-theme="dark"]` block, `var()` chains resolved) into `src/generated/palette.ts`; the generated file is the one place a literal may live in the package, `check-literals` scans the rest, and a test regenerates and compares so it cannot drift. Five kinds: `signUpCode`, `signInCode`, `emailChangeCode`, `share.member`, `share.invite` — no welcome, passkey or mention mail, because the PRD sends none. The catalogue is the web app's mechanism (same keys in six files, `{placeholders}` checked, product names untranslated); subjects are capped at 60 on a grapheme boundary.
- **Cognito's codes go through a custom-message trigger**, not a custom email sender: the built-in sender keeps delivery, bounces and the sandbox exemption Cognito's own address enjoys, and the trigger only shapes the message. It renders `signUpCode` for `CustomMessage_SignUp` and `_ResendCode`, `signInCode` for `CustomMessage_Authentication` (Cognito's source for an email OTP or MFA code at sign-in), `emailChangeCode` for `_UpdateUserAttribute` and `_VerifyUserAttribute` (the client may write `email`, ADR-019), and returns anything else untouched. It reads the user's `locale` attribute. For **its own faults** it fails open — a render error or a malformed event returns the event as received — because a trigger that throws refuses the sign-in itself. That is the whole extent of "fails open": a response Cognito refuses is not something the handler can see. And Cognito refuses one with the built-in sender: the developer guide states that with `EmailSendingAccount` other than `DEVELOPER` a trigger response carrying `emailMessage` or `emailSubject` makes Cognito answer the caller of `SignUp` / `InitiateAuth` with `InvalidLambdaResponseException` — no code delivered, every email sign-up, EMAIL_OTP sign-in and email change refused, for everyone, from the moment the stage deploys (#158 review). The live pool is `COGNITO_DEFAULT`. **So the function is attached to the pool only under the `customMessageTrigger` context flag (`cdk.json`, default `false`), which is flipped in the same merge as `withSES`** — the one configuration the documentation says honours the response. The function, its log group and its `Errors` alarm exist either way, so the flip is one property on the pool; `stage.test.ts` asserts no `LambdaConfig.CustomMessage` by default and its presence with the flag. Under it, the pool's own templates — `VerificationMessageTemplate` and the L1 `EmailAuthenticationMessage`/`Subject` — carry the same layout in en-US, rendered at synth, so what ships now is branded en-US codes from Cognito's address. The function has no IAM beyond its log group and makes no call. It is a `NodejsFunction` bundled with `@gede/mail`'s source by alias, so no stale dist can reach production. Code mails are stripped of format characters (the te-IN ZWNJ) because Cognito validates templates and trigger responses against `MessageTemplateType`'s pattern, which admits letters, marks, symbols, numbers, punctuation and whitespace only.
- **The web app writes the `locale` attribute**: at sign-up (so the confirmation code itself arrives in the device's language), on every change in the account menu beside `PATCH /api/me`, and once per session when the attribute differs from the account's choice. Share mail follows `users.locale` — the member's for `share.member`, the inviter's for `share.invite` (the address has no account to ask) — and en-US when neither is set.
- **The SPA client's `authSessionValidity` is ten minutes**: the sign-in screen and the mail both say the code expires in 10 minutes (AUTH-06); Cognito's default of three made that copy untrue. The sign-up code is Cognito's 24-hour verification code and the mail says so.
- **The sender stays `withCognito()`** until SES has production access (runbook §5); the templates are sender-independent and `withSES` changes only the From address.

**Consequences.** `packages/mail` is a root workspace and a project reference of `services/sync` and `infra`; `cdk synth` under `tsx` needs its dist (`npm run verify`'s `tsc -b` builds it; the infra and sync tests alias the source). A token change is followed by `npm run generate -w packages/mail` and a snapshot review — the `.html`/`.txt` snapshots per kind × locale are the rendered mail and are read like a template diff; `docs/mail-previews/` is regenerated from them by `packages/mail/scripts/screenshots.mjs`. Until SES production access is granted, every one-time code is the branded en-US pool template and the six locales apply to share mail alone; the trigger's localisation goes live with `withSES` + `customMessageTrigger=true` in one merge (runbook §5), never under `COGNITO_DEFAULT`. Once attached, a fault in the handler itself is a stock-looking en-US mail, never an outage, and the alarm reports it. The one Cognito event this pool can raise that the trigger does not render is none: admin-created users are created with their message suppressed and account recovery is off.
