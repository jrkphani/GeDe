...
| Table | Columns | Notes |
|---|---|---|
| `users` | id uuid pk · cognito_sub text unique · email citext unique · display_name text · locale text null · created_at · last_seen_at · tour_done_at null · deleted_at null · library_sort text null | Created on first sign-in from the JWT. No credentials stored. `locale` added by migration 0001 (I18N-05). `tour_done_at` added by migration 0009 (ONB-03): when the guided tour was completed or skipped, per account; cleared by Replay (ONB-08). `deleted_at` added by migration 0010 (ADR-038): set by account erasure; the row stays as a tombstone with every personal column nulled and `cognito_sub` kept, so foreign keys resolve and the identity cannot return. `library_sort` added by migration 0011 (LIB-05, #133): the Browse / Shared sort the account chose, `name` or `date` (`users_library_sort_check`), per account, never per device. |
| `documents` | id uuid pk · owner_id → users · title text · link_access enum(none, view, edit) · link_token text unique null · snapshot_key text null · snapshot_seq bigint · created_at · updated_at · deleted_at null · archived_at null · ever_shared boolean · sample boolean | One row per workbook. `snapshot_key` points into S3 `docs`. `created_at` added by migration 0002 (LIB-02 shows and sorts by it; the C4 listed only `updated_at`). Migration 0008 (LIB-D): `archived_at` (Archive, no expiry; never set with `deleted_at` — a CHECK enforces it), `ever_shared` (maintained by the service in the share transactions; Delete is refused while true), `sample` (the guided sample, exempt from Delete and Archive). Migration 0009 (ONB-01): partial unique index `documents_owner_sample_key (owner_id) WHERE sample` — one sample per owner, seeded by the service on the account's first request. |
...
| `invites` | id uuid pk · document_id · email citext · permission · token text unique · expires_at · accepted_at null · invited_by → users null · created_at · mail_sent_at null | For emails without an account yet; converts to a share on first sign-in (the share's `invited_by` is the invitation's, else the owner). `invited_by` and `created_at` added by migration 0006 with an index on `email` for the conversion; migration 0007 adds `invites_pending_key`, one pending invitation per address per document. `mail_sent_at` added by migration 0011 (#121): when SES last accepted the mail, null while it never did — the row is the grant either way; the sheet shows null as "email not sent" with Resend. |
