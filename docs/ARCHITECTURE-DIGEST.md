...
| Table | Columns | Notes |
|---|---|---|
| `users` | id uuid pk · cognito_sub text unique · email citext unique · display_name text · locale text null · created_at · last_seen_at · tour_done_at null | Created on first sign-in from the JWT. No credentials stored. `locale` added by migration 0001 (I18N-05). `tour_done_at` added by migration 0009 (ONB-03): when the guided tour was completed or skipped, per account; cleared by Replay (ONB-08). |
