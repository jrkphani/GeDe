...
| `invites` | id uuid pk · document_id · email citext · permission · token text unique · expires_at · accepted_at null · invited_by → users null · created_at | For emails without an account yet; converts to a share on first sign-in (the share's `invited_by` is the invitation's, else the owner). `invited_by` and `created_at` added by migration 0006 with an index on `email` for the conversion. |
