...
| Segmented control | molecule | Switch a view. Track #eef2f0 radius 7, active pill white with raised shadow. (Ring / Coverage) | Radix ToggleGroup |
...
2. Library (`inLibrary`): sidebar 230px (hidden <900px) with nav Recents ◷ / Browse ▭ / Shared ⇪ / Recently Deleted ⌫ and favourites (1Cloudhub, Governance); search; grouped list by when/owner; sort by Name/Date; participants panel (Owner, "(You)", Can make changes / View only). Fixture `LIBDOCS`: 1Cloudhub - Workscape (956 KB, Meenarapan), Generic Design Architect (Sembian V), GEDE | Resource library, GeDe | MicroWorks, REKI | JOTTER, Documentations | ResorucM2, Monthly Cash Distribution (mine), Everest trek.
