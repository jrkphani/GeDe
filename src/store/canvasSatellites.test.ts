...
import { resetCanvasSatellites, useCanvasSatellitesStore } from './canvasSatellites'
...
describe('canvasSatellites store (issue 011 / P3 / 106-②)', () => {
  ...
  it('starts empty with no camera state', () => {
    const s = useCanvasSatellitesStore.getState()
    expect(s.open).toEqual([])
    expect(s).not.toHaveProperty('focus')
  })

  it('openSatellite adds a parent context without carrying a pan target', () => {
  ...
  it('is idempotent — re-opening an already-open context does not duplicate or move it', () => {
    const store = useCanvasSatellitesStore.getState()
