...
export const configSchema = z
  ...
  .refine((c) => c.WS_BYTES_BURST >= c.WS_MAX_UPDATE_BYTES, {
    // A frame the bytes bucket could never pay for would close every socket
    // that sends one, whatever the rate: the burst must cover one full frame.
    message: 'WS_BYTES_BURST must be at least WS_MAX_UPDATE_BYTES',
    path: ['WS_BYTES_BURST'],
  })
  .refine((c) => c.DOC_LOG_MAX_BYTES <= c.DOC_MAX_BYTES, {
    message: 'DOC_LOG_MAX_BYTES must not exceed DOC_MAX_BYTES',
    path: ['DOC_LOG_MAX_BYTES'],
  });
