...
4. A new document's initial state (`seedNewDocument` in `@gede/core`: meta + one sheet) is written by `POST /api/documents` as snapshot seq 1, in the same transaction as the row. Clients never seed. The guided sample (ONB-01, `seedSampleWorkscape`) is written the same way by `SampleSeeder` (`sample.ts`) from the auth hook on an account's first request; `documents_owner_sample_key` keeps it to one per owner.
