...
- Sharing routes live in `routes/share.ts` (`README.md` lists them). Rules: any participant reads the sheet (a viewer gets names only); the owner and editors invite; the owner alone changes permissions, removes people, sets the link mode and stops sharing. A permission is frozen on a socket at upgrade, so a change closes that user's sockets (`RoomManager.closeUser`: 4403 when removed, 1001 to reconnect and re-resolve otherwise). Every share change writes its `audit_log` row (`share.*`) in the same transaction. Invitations have their own per-user budget (`RATE_LIMIT_INVITES_PER_HOUR`) counted after validation and the permission check.
