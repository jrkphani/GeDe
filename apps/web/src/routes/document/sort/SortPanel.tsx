...
export type OrganizeSection = 'categories' | 'sort' | 'filter';
...
export interface SortPanelProps {
  ...
  /**
   * INSP-01: which Organize tab this panel is the body of. Each tab shows its
   * own section (Categories, Sort or Filter); absent, every section renders.
   */
  section?: OrganizeSection | undefined;
}
 ...
 */
export function SortPanel({ gd, tableId, commands, section }: SortPanelProps) {
  ...
  const shows = (which: OrganizeSection) => section === undefined || section === which;
  const orderReason =
    disabledReason ?? (sortBy === null ? 'pick a column to sort by first' : undefined);

  return (
    <div className="gd-sort-panel" data-testid="sort-panel" data-section={section}>
      {disabledReason !== null && (
      ...
      {shows('filter') && (
        <section className="gd-sort-panel__section" aria-labelledby={filterHeadingId}>
          ...
          {tableId !== null && !disabled && (
            // INSP-12: live — every change writes the viewer's filter at once, no Apply step. The
            // form keeps its identity across its own writes (keyed by table and scope only), so
            // the field never loses focus or its caret while the person types.
            <FilterForm
              key={`${tableId}:${filterScope ?? ''}`}
              columnLabel={scopeLabel}
              initial={filter ?? { colId: filterScope, text: '', fuzzy: true, facet: null }}
              onChange={(next, settled) => {
                commands.setFilter(tableId, { ...next, colId: filterScope }, { announce: settled });
              }}
              onClear={() => {
