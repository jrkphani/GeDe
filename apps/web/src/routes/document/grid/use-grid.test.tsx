...
describe('column rename and derived lineage (REF-04)', () => {
  ...
  it('INSP-04 KEYS-03 SHARE-01 setTableTitle and renameColumn trim, refuse an empty name, write nothing for an unchanged one, are one undo step each and reach the other replica; a pulled or mapping column keeps its lineage name (ADR-051)', () => {
    ...
    const regions = createTable(a, {
      sheetId: tableById(a, tableId)!.sheetId,
      at: { col: 8, row: 0 },
      columns: 1,
      rows: 1,
    });
    ...
    expect(tableById(a, tableId)?.columns[1]?.label).toBe('Column 2');
    expect(tableById(a, tableId)?.title).toBe('Table 1');
  });
