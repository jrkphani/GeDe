...
Mail tables (migration 0012, ADR-046; written only by the SES events poller in `services/sync`): `mail_events`, `mail_suppressions`.
...
- The ledger records the SHA-256 of every applied file (migration 0005, `checksum`). A file whose text differs from its row fails the boot with the file named; rows from before 0005 are pinned on the next run. The check runs in both directions (#110): a ledger row with no file on disk (a removed or renamed shipped migration) fails the boot too, as does a new file that sorts before the newest applied row. The one tolerated mismatch is a ledger row newer than every file — a rolled-back deploy booting the previous image — which is logged. So: never edit, remove or rename a shipped file, not even a comment — write a new one, numbered after the last.
- With `appRole` (`PGAPPUSER`/`PGAPPPASSWORD` in production), the runner bootstraps the least-privilege runtime role under the same lock before the first file (#36, ADR-022): idempotent `CREATE ROLE`/`ALTER ROLE … PASSWORD`, DML on every table, `USAGE` on sequences, default privileges for whatever later migrations create, `REVOKE CREATE ON SCHEMA public FROM PUBLIC`, read-only on `__migrations`. It is a runner step and not a migration file because it must re-run on every boot (password rotation, re-grants). The master user is `rds_superuser`, not a superuser: never add `SUPERUSER`/`REPLICATION`/`BYPASSRLS` to the `ALTER ROLE`. `db:parity` runs the bootstrap as an equivalent non-superuser CREATEROLE database owner (never the container's superuser, which would hide exactly that mistake), then connects as the role and asserts what it can and cannot do; extend that check when the runtime needs a new privilege. The `ALTER ROLE … PASSWORD` sits in its own `EXCEPTION` block so a failure is re-raised without the statement text: the server log must never carry the password literal.
- Every foreign key a cascade or the projection walks has an index (migration 0004); declare new ones in `schema.ts` as well, `migrations.test.ts` and `db:parity` check both directions. Every CHECK is declared in `schema.ts` under the name the SQL gives it (0008 `documents_archived_or_deleted_check`, 0010 `documents_sample_not_deleted_check`, 0011 `users_library_sort_check`, 0012 `mail_events_kind_check` and `mail_suppressions_reason_check`); `db:parity` requires each to exist.
...
- Exports: `schema` (Drizzle tables and enums), `applyMigrations(pool, dir, options)`, `createPool(env, options)` (TLS `verify-full` with the RDS CA bundle from `PGSSLROOTCERT`; with `NODE_ENV=production` any other `PGSSLMODE` is refused; `{ as: appRole }` connects as the runtime role instead of `PGUSER`), and `appRoleFromEnv(env)` (`PGAPPUSER`/`PGAPPPASSWORD`; `undefined` locally, an error in production when unset or half-set). Every pooled session carries `statement_timeout` 30 s, `lock_timeout` 10 s and `idle_in_transaction_session_timeout` 30 s (`POOL_TIMEOUTS`).
